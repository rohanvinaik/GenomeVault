...
class TopologyRequest(BaseModel):
    ...
    location: Optional[Dict[str, float]] = Field(
        None, 
        description="User location for nearest node discovery"
    )
    node_requirements: Optional[Dict[str, Any]] = Field(
        None,
        description="Specific node requirements"
    )


class TopologyResponse(BaseModel):
    """Network topology response."""
    nearestLNs: List[str] = Field(
        description="List of nearest light node IDs"
    )
    tsNodes: List[str] = Field(
        description="List of trusted signatory node IDs"
    )
    optimal_configuration: Optional[Dict] = Field(
        None,
        description="Optimal node configuration for PIR"
    )
...
async def redeem_credits(
    ...
    try:
        ...
        tx_id = hashlib.sha256(
            f"{request.invoiceId}:{time.time()}".encode()
        ).hexdigest()[:16]
...
@app.get("/pir/config")
async def get_pir_configuration(
    api_key: str = Depends(verify_api_key)
):
