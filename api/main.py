...
async def redeem_credits(request: CreditVaultRequest,
                        user_id: str = Depends(verify_token)):
    """
    ...
    """
    logger.info(f"Credit redemption: {request.creditsBurned} credits for invoice {request.invoiceId}",
...
async def create_audit_challenge(request: AuditChallengeRequest,
                               user_id: str = Depends(verify_token)):
    """
    ...
    """
    logger.info(f"Audit challenge from {request.challenger} to {request.target}",
               extra={'privacy_safe': True})
...
async def create_processing_pipeline(request: PipelineRequest,
                                   user_id: str = Depends(verify_token)):
    """
...
async def perform_vector_operation(request: VectorRequest,
                                 user_id: str = Depends(verify_token)):
    """
...
async def generate_proof(request: ProofRequest,
    ...
    """
    logger.info(f"Proof generation request: {request.circuit_name}", 
               extra={'privacy_safe': True})
