#!/usr/bin/env python3
"""
Optimized GenomeVault Pipeline with Performance Improvements

Runs the complete pipeline with all safe optimizations:
...
Compares performance with baseline run.
"""
...
def run_optimized_pipeline(preset: str = "production", enable_optimizations: bool = True):
    ...
    run_dir = OUTPUT_DIR / f"pipeline_run_optimized_{timestamp}"
    run_dir.mkdir(parents=True, exist_ok=True)

    logger.info(f"Starting OPTIMIZED pipeline run: {run_dir}")
...
if __name__ == "__main__":
    ...
    if args.compare and BASELINE_RESULTS.exists():
