    ...
    for protocol in protocols:
        ...
        for i in range(10):
            coordinator.register_participant(
                f"participant_{i}", data_size=1000, compute_capacity=1.0
            )
        ...
        if result["status"] == "success":
            print(
                f"    ✓ {protocol.value}: Round completed with {result['participants']} participants"
            )
...
def test_differential_privacy():
    ...
    for epsilon in epsilons:
        ...
        privacy_params = PrivacyParameters(
            epsilon=epsilon, delta=1e-5, clip_norm=1.0, noise_multiplier=1.0
        )

        config = TrainingConfig(
            num_rounds=5, min_participants=3, protocol=AggregationProtocol.SECURE_AGG
        )
...
def test_participant_selection_fairness():
    ...
    for p in participants_config:
        coordinator.register_participant(
            p["id"], p["data_size"], p["capacity"], geographic_region=f"region_{hash(p['id']) % 3}"
        )
    ...
    for p_id, count in selection_counts.items():
        ...
        print(
            f"    {p_id}: {count}/20 rounds (data={participant.data_size}, "
...
def test_dropout_tolerance():
    ...
    config = TrainingConfig(
        min_participants=10, dropout_tolerance=0.3, protocol=AggregationProtocol.SECURE_AGG
    )
    ...
    dropped_count = sum(1 for p in coordinator.participants.values() if p.status.value == "dropped")
    print("    ⚠ 40% dropout exceeded threshold")
...
def test_convergence_detection():
    ...
    config = TrainingConfig(
        num_rounds=100, convergence_threshold=1e-4, checkpoint_interval=5, evaluation_interval=2
    )
...
def test_privacy_accountant():
    ...
    for round_num in range(20):
        ...
        if result["status"] == "success":
            ...
            print(
                f"    Round {round_num}: ε_consumed={coordinator.privacy_accountant.consumed_epsilon:.2f}, "
...
def test_model_checkpointing():
    ...
    config = TrainingConfig(
        num_rounds=20, checkpoint_interval=5, protocol=AggregationProtocol.MASKED
    )
...
def test_multi_party_training_simulation():
    ...
    privacy_params = PrivacyParameters(epsilon=1.0, delta=1e-5, clip_norm=1.0)
    ...
    print(
        f"    Privacy consumed: ε={training_summary['privacy']['consumed_budget']['epsilon']:.2f}"
    )
