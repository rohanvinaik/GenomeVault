...
def test_pairwise_masks():
    ...
    aggregator = SecureAggregator(num_clients=num_clients, vector_size=vector_size, seed=123)
...
def test_fedavg_with_dp():
    ...
    print("  Privacy level: COMMON (ε=10, δ=1e-5)")
...
def test_combined_privacy():
    ...
    if response.details["differential_privacy"]:
        print(
            f"  DP parameters: ε={response.details['dp_epsilon']}, "
...
def test_privacy_under_dropout():
    ...
    for i in range(participating_clients):
        weights = np.random.randn(vector_size) * 0.1
        update = ModelUpdate(
            client_id=f"client_{i}", weights=weights.tolist(), num_examples=100, metrics={}
        )
        updates.append(update)
    ...
    print("  Privacy maintained: Yes (via DP)")
...
def analyze_privacy_utility_tradeoff():
    ...
    for i in range(num_clients):
        update = ModelUpdate(
            client_id=f"client_{i}", weights=true_update.tolist(), num_examples=100, metrics={}
        )
        updates.append(update)
    ...
    for name, level, epsilon, delta in privacy_configs:
        if epsilon is None:
            ...
            aggregator = FedAvgAggregator(
                use_differential_privacy=True, privacy_epsilon=epsilon, privacy_delta=delta
            )
