...
try:
    from genomevault.pir import (
        PIRProtocol,
        ITPrivateInformationRetrieval,
        ITPIRProtocol
    )
    print("  ✅ All names imported from genomevault.pir")
    print(f"     All are same class? {PIRProtocol is ITPrivateInformationRetrieval is ITPIRProtocol}")
except ImportError as e:
    print(f"  ❌ Failed: {e}")
...
try:
    ...
    print(f"     Query shape: {queries[0].shape if hasattr(queries[0], 'shape') else len(queries[0])}")
