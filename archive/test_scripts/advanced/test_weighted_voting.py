...
def test_node_weights():
    ...
    for name, resource, signatory, expected in test_cases:
        node = DualAxisNode(
            node_id=name.replace(" ", "_"), resource_class=resource, signatory_status=signatory
        )
...
def test_hipaa_verification():
    ...
    node3 = DualAxisNode(node_id="regular_node", resource_class=ResourceClass.LIGHT)
...
def test_bft_safety():
    ...
    for byz_ratio, expected_safe, description in test_scenarios:
        ...
        print(
            f"{byz_ratio*100:10.0f}% | {consensus.total_weight:7} | "
...
def test_credit_rewards():
    ...
    for node in test_nodes:
        ...
        print(
            f"{node_type:12} | {c} | {'Yes' if signatory_bonus else 'No':3} | "
...
def test_slashing():
    ...
    node = DualAxisNode(node_id="test_node", resource_class=ResourceClass.FULL, stake=1000.0)
    ...
    for i, (percentage, expected) in enumerate(slashing_events, 1):
        ...
        print(
            f"{i:7} | {percentage*100:10.0f}% | {expected:8.2f} | "
...
def test_consensus_scenarios():
    ...
    for scenario in scenarios:
        ...
        print(
            f"{scenario['name']:20} | {scenario['num_nodes']:5} | "
...
def analyze_weight_distribution():
    ...
    print(
        f"  Consensus threshold: {consensus.threshold} ({consensus.threshold/total_weight*100:.1f}%)"
    )
