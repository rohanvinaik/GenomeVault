...
class CompletePipelineTest:
        ...
        try:
            ...
            for dim in dimensions:
                ...
                results[f"{dim}D"] = {
                    ...
                    "metal_acceleration": (
                        encoder.backend_type if hasattr(encoder, "backend_type") else "detected"
                    ),
                }
    ...
    def test_4_pir_variable_length_with_it_protocol(self) -> bool:
        ...
        try:
            ...
            print(
                f"  ✅ Query accuracy: {sum(r['correct'] for r in retrieval_results)}/{len(retrieval_results)}"
            )
        ...
        try:
            ...
            if variant_circuit:
                circuit_files = {
                    "r1cs_exists": variant_circuit.r1cs_path.exists(),
                    "r1cs_size": (
                        variant_circuit.r1cs_path.stat().st_size
                        if variant_circuit.r1cs_path.exists()
                        else 0
                    ),
                    "wasm_exists": variant_circuit.wasm_path.exists(),
                    "wasm_size": (
                        variant_circuit.wasm_path.stat().st_size
                        if variant_circuit.wasm_path.exists()
                        else 0
                    ),
                    "zkey_exists": variant_circuit.zkey_path.exists(),
                    "vkey_exists": variant_circuit.vkey_path.exists(),
        ...
        try:
            ...
            for i in range(3):
                ...
                if hasattr(proof, "metadata") and proof.metadata:
                    ...
                    proof_metrics.append({"proof_index": i, "performance": perf, "safety": safety})
            ...
            results = {
                ...
                "monitoring_features": {
                    ...
                    "performance_metadata": len(proof_metrics) > 0
                    and "performance" in proof_metrics[0],
            ...
            print("  ✅ Proof metadata: performance + safety tracking")
    ...
    def generate_comprehensive_summary(self) -> dict:
        ...
        if zk_test.get("status") == "PASS" and "details" in zk_test:
            performance_summary["zk_proofs"] = {
                ...
                "backend_type": zk_test["details"]
                .get("safety_metadata", {})
                .get("backend_type", "unknown"),
            }
        ...
        if parallel_test.get("status") == "PASS" and "details" in parallel_test:
            performance_summary["parallel_proving"] = {
                ...
                "success_rate": (
                    ...
                    / parallel_test["details"].get("total_tasks", 1)
                    * 100
                ),
            }
        ...
        feature_status = {
            ...
            "pir_variable_length": self.results["tests"]
            .get("PIR_Variable_Length_IT_Protocol", {})
            .get("status")
            == "PASS",
            "hardware_acceleration": self.results["tests"]
            .get("Hardware_Acceleration_Unified", {})
            .get("status")
            == "PASS",
            "circom_compilation": self.results["tests"]
            .get("Circom_Compilation_Infrastructure", {})
            .get("status")
            == "PASS",
            "performance_monitoring": self.results["tests"]
            .get("Comprehensive_Performance_Monitoring", {})
            .get("status")
            == "PASS",
            "production_safety": self.results["tests"]
            .get("Production_Safety_Integration", {})
            .get("status")
            == "PASS",
        }
    ...
    def run_complete_test_suite(self):
        ...
        tests = [
            ...
            (
                "Real ZK Proofs + Powers of Tau Ceremony",
                self.test_2_real_zk_proofs_with_tau_ceremony,
            ),
            (
                "Parallel Proving + Performance Monitoring",
                self.test_3_parallel_proving_with_performance_monitoring,
            ),
            (
                "PIR Variable Length + IT-PIR Protocol",
                self.test_4_pir_variable_length_with_it_protocol,
            ),
            (
                "Hardware Acceleration + Unified Engine",
                self.test_5_hardware_acceleration_unified_engine,
            ),
            (
                "Circom Compilation Infrastructure",
                self.test_6_circom_compilation_and_infrastructure,
            ),
            (
                "Comprehensive Performance Monitoring",
                self.test_7_comprehensive_performance_monitoring,
            ),
        ...
        for test_name, test_func in tests:
            ...
            try:
                ...
                self.log_test(
                    test_name.replace(" ", "_").replace("+", "_"), False, {"crash": str(e)}
                )
            print()
        ...
        print(
            f"All Features Working: {'✅ YES' if summary['overall']['all_features_working'] else '❌ NO'}"
        )
        print()
        ...
        if summary["performance_summary"]:
            ...
            if "hdc" in summary["performance_summary"]:
                print(
                    f"  • HDC Encoding: {summary['performance_summary']['hdc']['fastest_encoding']:.2f}ms"
                )
            if "zk_proofs" in summary["performance_summary"]:
                zk_perf = summary["performance_summary"]["zk_proofs"]
                print(
                    f"  • ZK Proof Generation: {zk_perf['proof_generation_ms']:.2f}ms ({zk_perf['backend_type']} backend)"
                )
