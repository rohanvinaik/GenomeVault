...
def test_hypervector_with_unified():
    ...
    config = HypervectorConfig(dimension=10000, batch_size=1024, use_sparse=True)
...
def benchmark_comparison():
    ...
    for acc in accelerators:
        ...
        try:
            ...
            config = AccelerationConfig(dimension=10000, device=acc.type)
            ...
            print(
                f"{acc.type.value:8s}: {bench_results.get('time_ms', 0):.2f}ms, "
...
def test_integration_with_existing():
    ...
    from genomevault.hypervector_transform.encoding import (
        HypervectorEncoder,
        HypervectorConfig as OldConfig,
    )
    ...
    print(
        f"Old encoder shape: {old_result.shape if hasattr(old_result, 'shape') else len(old_result)}"
    )
