...
class HIPAACompliantUser(HttpUser):
    ...
    def initialize_privacy_budget(self):
        ...
        response = self.client.post(
            "/privacy/budget/init",
            json={"user_id": self.user_id, "epsilon": DIFFERENTIAL_PRIVACY_EPSILON, "delta": 1e-5},
            catch_response=True,
        )

        if response.status_code != 200:
            response.failure("Failed to initialize privacy budget")
    ...
    def query_genomic_variant(self):
        ...
        with self.client.get(
            f"/genomic/variant/{variant_id}", name="/genomic/variant/[id]", catch_response=True
        ) as response:
            if response.status_code == 200:
                self.query_count += 1
    ...
    def hypervector_encoding(self):
        ...
        with self.client.post(
            "/hv/encode",
            json={"variants": variants, "dimension": 10000, "compression_tier": COMPRESSION_TIER},
            name="/hv/encode",
            catch_response=True,
        ) as response:
    ...
    def clinical_data_query(self):
        ...
        with self.client.get(
            "/clinical/cohort", params=query_params, name="/clinical/cohort", catch_response=True
        ) as response:
            if response.status_code == 200:
                data = response.json()
    ...
    def zk_proof_verification(self):
        ...
        with self.client.post(
            "/zk/verify", json=proof_data, name="/zk/verify", catch_response=True, timeout=5
        ) as response:
            if response.status_code == 200:
                data = response.json()
                ...
                if response.elapsed.total_seconds() > 0.5:
                    response.failure(
                        f"ZK verification too slow: {response.elapsed.total_seconds()}s"
                    )
                else:
                    response.success()
            else:
...
def on_test_stop(environment, **kwargs):
    ...
    print(
        f"  Availability ≥ {availability_target:.1%}: {'✓ PASS' if slo_availability else '✗ FAIL'} ({availability_actual:.3%})"
    )
