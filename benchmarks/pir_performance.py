...
class PIRPerformanceBenchmark:
    ...
    def run_benchmarks(self) -> List[BenchmarkResult]:
        ...
        """
        print(f"Starting PIR Performance Benchmarks")
    ...
    def _benchmark_database(self, database: np.ndarray, db_size: int) -> BenchmarkResult:
        ...
        # Progress bar
        pbar = tqdm(total=self.config.num_queries + self.config.warmup_queries,
                   desc="Queries", leave=False)
    ...
    def _generate_plots(self) -> None:
        ...
        for i, result in enumerate(self.results):
            if i % 2 == 0:  # Plot every other result for clarity
                ax5.hist(result.total_time_ms, bins=30, alpha=0.5, 
                        label=f'{db_sizes[i]:.1f}GB')
...
def main():
    ...
    parser.add_argument('--database-sizes', nargs='+', type=int,
                       help='Database sizes in blocks')
    parser.add_argument('--num-queries', type=int, default=100,
                       help='Number of queries per benchmark')
    parser.add_argument('--block-size', type=int, default=1024,
                       help='Block size in bytes (default: 1024 for 1KB)')
    parser.add_argument('--num-servers', type=int, default=2,
                       help='Number of PIR servers')
    parser.add_argument('--output-dir', type=str, default='benchmark_results',
                       help='Output directory for results')
    parser.add_argument('--disable-byzantine', action='store_true',
                       help='Disable Byzantine fault tolerance')
    parser.add_argument('--disable-constant-time', action='store_true',
                       help='Disable constant-time execution')
