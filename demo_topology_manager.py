...
def simulate_pir_queries(manager, num_queries=20):
    ...
    for i in range(num_queries):
        ...
        print(f"Configuration: {config.total_shards} shards "
              f"({config.light_nodes} LN + {config.trusted_signatories} TS)")
        print(f"Selected servers:")
        for server in servers:
            print(f"  • {server.id:15} ({server.type.value}): "
        ...
        if random.random() < 0.1:  # 10% chance of network congestion
            ...
            print(f"⚠️  Network congestion detected!")
        else:
        ...
        if not success:
            print(f"❌ Query failed - handling failure...")
        ...
        print(f"Latency: {actual_latency:.1f}ms "
              f"(expected: {config.expected_latency_ms:.1f}ms)")
...
def main():
    ...
    print(f"   • Trusted Signatories: {sum(1 for s in servers if s.type == ServerType.TRUSTED_SIGNATORY)}")
    ...
    for name, preset in AdaptiveTopologyManager.PRESET_CONFIGS.items():
        print(f"\n   {name}:")
        print(f"      Shards: {preset.total_shards} ({preset.light_nodes} LN + {preset.trusted_signatories} TS)")
    ...
    if stats['average_latencies']:
        ...
        for server_id, avg_latency in sorted(stats['average_latencies'].items(), 
                                            key=lambda x: x[1]):
