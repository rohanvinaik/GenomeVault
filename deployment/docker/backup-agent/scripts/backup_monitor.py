...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("/var/log/backup_monitor.log"), logging.StreamHandler()],
)
...
backup_duration_seconds = Histogram(
    "genomevault_backup_duration_seconds", "Backup duration", ["type"]
)
restore_test_results = Gauge(
    "genomevault_restore_test_success", "Restore test success (1) or failure (0)"
)
data_integrity_checks = Counter(
    "genomevault_data_integrity_checks_total", "Data integrity checks", ["status"]
)
...
class BackupMonitor:
    ...
    def monitor_backup(self, backup_type: str, backup_path: str) -> Dict:
        ...
        try:
            ...
            # Send alert
            self._send_alert(
                level="ERROR", title=f"Backup {backup_type} Failed", message=str(e), details=results
            )
    ...
    def _verify_encryption(self, file_path: str) -> bool:
        ...
        try:
            ...
            with open(file_path, "rb") as f:
                ...
                if header.startswith(b"-----BEGIN PGP MESSAGE-----") or header.startswith(
                    b"Salted__"
                ):
                    return True
    ...
    def test_restore(self, backup_path: str, test_database: str = "genomevault_test") -> bool:
        ...
        try:
            ...
            restore_cmd = (
                f"pg_restore -h {os.getenv('DATABASE_HOST')} -p {os.getenv('DATABASE_PORT')} "
    ...
    def cleanup_old_backups(self):
        ...
        try:
            cutoff_dates = {
                "incremental": datetime.utcnow()
                - timedelta(days=self.retention_days["incremental"]),
            ...
            self._send_alert(level="WARNING", title="Backup Cleanup Failed", message=str(e))
    ...
    def _cleanup_s3_backups(self, cutoff_dates: Dict[str, datetime]):
        ...
        try:
            for backup_type, cutoff_date in cutoff_dates.items():
                ...
                pages = paginator.paginate(Bucket=self.s3_bucket, Prefix=f"{backup_type}/")
                ...
                if objects_to_delete:
                    ...
                    logger.info(
                        f"Deleted {len(objects_to_delete)} old {backup_type} backups from S3"
                    )
    ...
    def _cleanup_local_backups(self, cutoff_dates: Dict[str, datetime]):
        ...
        for backup_type, cutoff_date in cutoff_dates.items():
            ...
            if type_dir.exists():
                ...
                if deleted_count > 0:
                    logger.info(
                        f"Deleted {deleted_count} old {backup_type} backups from local storage"
                    )
    ...
    def _send_alert(self, level: str, title: str, message: str, details: Optional[Dict] = None):
        ...
        try:
            color = {"ERROR": "danger", "WARNING": "warning", "INFO": "good"}.get(level, "default")
            ...
            self.slack_client.chat_postMessage(
                channel=os.getenv("SLACK_CHANNEL", "#genomevault-alerts"), attachments=[attachment]
            )
