...
def fix_union_syntax():
    ...
    repo_root = Path(".")
    files_to_fix = ["genomevault/core/config.py", "genomevault/core/exceptions.py"]
    ...
    for file_path in files_to_fix:
        ...
        if content != original_content:
            ...
            logger.info(f"✓ Fixed union syntax in {file_path}")

    if fixes_applied > 0:
        logger.debug(f"Applied {fixes_applied} union syntax fixes")
    else:
        logger.debug("No union syntax issues found")
