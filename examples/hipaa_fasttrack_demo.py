    ...
    async with CMSNPIRegistry() as registry:
        ...
        if registered_nodes:
            ...
            for provider, node in registered_nodes:
                choice = "yes" if "Hospital" in provider['name'] or "Lab" in provider['name'] else "abstain"
            ...
            for provider, node in registered_nodes:
                print(f"  {provider['name']:30} | Class: {node.node_type.name:7} | Power: {node.voting_power:3}")
   ...
   - Automated compliance tracking
        ...
        if registered_nodes:
            ...
            success = await integration.revoke_provider_node(
                npi=test_npi,
                reason="Demonstration of revocation process"
            )
