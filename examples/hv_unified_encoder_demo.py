...
def main():
    ...
    for chrom, pos, ref, alt, var_type in variants:
        ...
        logger.debug(f"  {var_type} at {chrom}:{pos} {ref}>{alt} -> vector shape: {vec.shape}")
    ...
    for i in range(len(variants)):
        for j in range(i + 1, len(variants)):
            ...
            logger.debug(f"  Variant {i + 1} vs Variant {j + 1}: {sim:.3f}")
    ...
    for seq in sequences:
        ...
        logger.debug(f"  '{seq}' -> vector shape: {vec.shape}")
    ...
    logger.debug(f"Genomic vector shape: {genomic_vec.shape}")
    logger.debug(f"Clinical vector shape: {clinical_vec.shape}")
    logger.debug(f"Combined vector shape: {combined_vec.shape}")

    # Example 4: Performance benchmark
    logger.debug("\n4. Performance benchmark")
    logger.debug("Testing encoding speed for 10,000 variants...")
    ...
    logger.debug(f"Average time per variant: {elapsed_time / n_variants * 1000:.3f} ms")
    ...
    for name, similarity in components:
        logger.debug(f"  {name}: similarity = {similarity:.3f}")
