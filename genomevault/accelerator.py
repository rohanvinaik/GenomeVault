...
class Accelerator:
    ...
    def batch_hypervector_similarity(
        self,
        vectors: np.ndarray,
        query: np.ndarray
    ) -> np.ndarray:
    ...
    def pir_xor_mask(
        ...
        """
        if self.use_rust:
            return genomevault_accel.fast_pir_xor_mask(
    ...
    def hamming_distance(
        ...
        """
        if self.use_rust:
            return genomevault_accel.fast_hamming_distance(
                a.astype(np.uint8),
                b.astype(np.uint8)
            )
        else:
    ...
    def batch_hamming_distance(
        ...
        if self.use_rust:
            return genomevault_accel.batch_hamming_distance(
                vectors.astype(np.uint8),
                query.astype(np.uint8)
            )
        else:
            # Python fallback
            distances = []
    ...
    def compress_hypervector(
        ...
        """
        if self.use_rust:
            return genomevault_accel.compress_hypervector_binary(
