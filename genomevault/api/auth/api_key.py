...
class APIKeyManager:
    ...
    def _load_keys(self):
        ...
        if prod_key:
            ...
            self._keys_cache[prod_key_hash] = APIKeyInfo(
                ...
                scopes={
                    ...
                    APIKeyScope.READ_HDC,
                    APIKeyScope.WRITE_HDC,
                    APIKeyScope.WRITE_ZK,
                    APIKeyScope.WRITE_PIR,
                    APIKeyScope.READ_METRICS,
                },
        ...
        if clinical_key:
            ...
            self._keys_cache[clinical_key_hash] = APIKeyInfo(
                ...
                scopes={
                    APIKeyScope.READ,
                    APIKeyScope.CLINICAL_READ,
                    APIKeyScope.CLINICAL_WRITE,
                    APIKeyScope.READ_HDC,
                    APIKeyScope.WRITE_HDC,
                },
    ...
    def validate_api_key(
        ...
        if not key_info.is_active:
            raise HTTPException(status_code=HTTP_401_UNAUTHORIZED, detail="API key is disabled")
        ...
        if client_ip and not key_info.is_ip_allowed(client_ip):
            ...
            raise HTTPException(
                status_code=HTTP_403_FORBIDDEN, detail="Access denied from this IP address"
            )

        # Check user agent pattern
        if (
            key_info.user_agent_pattern
            and user_agent
            and not self._matches_pattern(user_agent, key_info.user_agent_pattern)
        ):
            print(f"API key access denied for key_id={key_info.key_id}: User agent mismatch")
            raise HTTPException(
                status_code=HTTP_403_FORBIDDEN, detail="Access denied: invalid client"
            )
    ...
    def create_api_key(
        ...
        client_ip_whitelist: Optional[List[str]] = None,
...
async def get_api_key_from_request(
    ...
    bearer_creds: Optional[HTTPAuthorizationCredentials] = Depends(bearer_auth),
) -> Optional[str]:
...
async def authenticate_api_key(
    request: Request, api_key: Optional[str] = Depends(get_api_key_from_request)
) -> APIKeyInfo:
