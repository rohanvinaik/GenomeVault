...
def create_access_token(data: TokenData, expires_delta: Optional[timedelta] = None) -> str:
    ...
    redis_client.setex(token_key, timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES), encoded_jwt)
...
def create_refresh_token(
    user: User, session_id: str, ip_address: Optional[str] = None, user_agent: Optional[str] = None
) -> str:
    ...
    redis_client.setex(redis_key, timedelta(days=REFRESH_TOKEN_EXPIRE_DAYS), refresh_data.json())
...
def get_user_db(username: str) -> Optional[UserInDB]:
    ...
    if user:
        ...
        return UserInDB(**user.dict(), hashed_password=get_password_hash("genomevault123"))
    return None
    ...
    try:
        ...
        if refresh_data.used:
            ...
            logger.error(
                ...
                extra={"event": "token_reuse_detected", "username": username, "token_id": token_id},
            )
        ...
        logger.info(
            ...
            extra={"event": "token_refreshed", "username": username, "old_token_id": token_id},
        )
