...
class AIIntegrationSettings(BaseModel):
    ...
    openai_default_model: str = Field(
        "gpt-4o",
        env="GENOMEVAULT_OPENAI_MODEL",
        description="Default GPT model to use"
    )
    openai_max_tokens: int = Field(
        4096,
        env="GENOMEVAULT_OPENAI_MAX_TOKENS",
        description="Maximum tokens for GPT responses"
    )
    ...
    enable_pii_filtering: bool = Field(
        True,
        env="GENOMEVAULT_AI_PII_FILTERING",
        description="Enable PII filtering in AI requests"
    )
    enable_audit_logging: bool = Field(
        True,
        env="GENOMEVAULT_AI_AUDIT_LOGGING",
        description="Enable audit logging for AI requests"
    )
    ...
    enable_anthropic: bool = Field(
        True,
        env="GENOMEVAULT_ENABLE_ANTHROPIC",
        description="Enable Anthropic Claude integration"
    )
    enable_openai: bool = Field(
        True,
        env="GENOMEVAULT_ENABLE_OPENAI",
        description="Enable OpenAI GPT integration"
    )
