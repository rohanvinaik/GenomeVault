...
async def liveness_check() -> LivenessCheckResponse:
    ...
    """
    return LivenessCheckResponse(alive=True, timestamp=datetime.utcnow())
...
async def readiness_check() -> ReadinessCheckResponse:
    ...
    try:
        ...
        return ReadinessCheckResponse(ready=ready, timestamp=datetime.utcnow(), checks=checks)

    except Exception as e:
        logger.error(f"Readiness check failed: {e}")
        return ReadinessCheckResponse(
            ready=False, timestamp=datetime.utcnow(), checks={"error": False}
        )
...
async def _check_all_services() -> Dict[str, ServiceDetails]:
    ...
    check_tasks = [
        _check_service_with_timeout(name, check_func) for name, check_func in service_checks.items()
    ]
...
async def _check_service_with_timeout(
    name: str, check_func, timeout: float = 5.0
) -> ServiceDetails:
    """
...
async def _check_database_health() -> ServiceDetails:
    ...
    try:
        ...
        return ServiceDetails(
            status=ServiceStatus.UNHEALTHY, last_check=datetime.utcnow(), error_message=str(e)
        )
