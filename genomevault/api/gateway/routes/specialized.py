...
from genomevault.api.gateway.models.base import SuccessResponse
...
async def discover_topology(request: TopologyRequest) -> TopologyResponse:
    ...
    try:
        ...
        nearest_lns = [
            node for node in optimized_nodes 
            if node.node_type == NodeType.LIGHT_NODE
        ][:request.max_nodes // 2]
        
        ts_nodes = [
            node for node in optimized_nodes
            if node.node_type == NodeType.TRUSTED_SERVER
        ][:request.max_nodes // 2]
        ...
        optimization_score = await _calculate_optimization_score(
            nearest_lns + ts_nodes, request
        )
...
async def submit_audit_challenge(
    ...
    try:
        ...
        background_tasks.add_task(
            _notify_verification_nodes, verification_nodes, challenge_id
        )
...
async def get_challenge_status(
    challenge_id: str,
    request: ChallengeStatusRequest = ChallengeStatusRequest(challenge_id="")
) -> ChallengeStatusResponse:
    """
...
async def _optimize_node_selection(nodes: List[NetworkNode], request: TopologyRequest) -> List[NetworkNode]:
...
async def _calculate_optimization_score(nodes: List[NetworkNode], request: TopologyRequest) -> float:
...
async def _select_failover_nodes(available_nodes: List[NetworkNode], selected_nodes: List[NetworkNode]) -> List[NetworkNode]:
...
async def _process_credit_redemption(vault_id: str, credit_type: CreditType, amount: int, invoice_id: str) -> str:
...
async def _update_vault_balance(vault_id: str, credit_type: CreditType, delta: int) -> dict[CreditType, int]:
...
async def _generate_audit_trail(request: CreditRedemptionRequest, invoice_id: str, credits_burned: int) -> str:
...
async def _store_challenge(challenge_id: str, request: AuditChallengeRequest, result_hash: str, verification_nodes: List[str]):
...
async def _notify_verification_nodes(verification_nodes: List[str], challenge_id: str):
    ...
    logger.info(f"Notified {len(verification_nodes)} verification nodes about challenge {challenge_id}")