...
class ConnectionManager:
    ...
    async def connect(
        ...
        self.connection_stats[connection_id] = {"messages_sent": 0, "messages_received": 0}
        ...
        logger.info(
            ...
            extra={
                ...
                "connection_duration_seconds": (
                    (datetime.utcnow() - conn_info.connected_at).total_seconds() if conn_info else 0
                ),
            },
        )
    ...
    async def handle_pong(self, connection_id: str, message: WebSocketMessage):
        ...
        if connection_id in self.pending_pings:
            ...
            logger.debug(
                ...
                extra={"connection_id": connection_id, "response_time_ms": response_time * 1000},
            )
    ...
    def get_statistics(self) -> Dict[str, any]:
        ...
        return {
            ...
            "average_messages_per_connection": {
                ...
                "received": (
                    total_messages_received / len(self.connections) if self.connections else 0
                ),
            },
        }
            ...
            try:
                ...
                for connection_id in list(self.pending_pings.keys()):
                    ...
                    if time.time() - ping_time > 60:  # No response for 1 minute
                        logger.warning(
                            f"Connection {connection_id} not responding to pings, disconnecting"
                        )
                        await self.disconnect(connection_id)
