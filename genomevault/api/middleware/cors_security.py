...
class OriginValidator:
    ...
    def __init__(
        ...
        self.strict_https_only = security_level in [
            CORSSecurityLevel.STRICT,
            CORSSecurityLevel.CLINICAL,
        ]
        self.allow_localhost = security_level in [
    ...
    def __init__(
        ...
        if allowed_headers is None:
            ...
            if security_level in [CORSSecurityLevel.PERMISSIVE, CORSSecurityLevel.STANDARD]:
                base_headers.extend(["X-Requested-With", "X-CSRF-Token", "Cache-Control"])
    ...
    async def _handle_preflight(self, request: Request, origin: Optional[str]) -> Response:
        ...
        if origin:
            ...
            if not allowed:
                ...
                return PlainTextResponse("CORS preflight denied", status_code=HTTP_403_FORBIDDEN)

        # Check if method is allowed
        requested_method = request.headers.get("access-control-request-method")
        if requested_method and requested_method not in self.allowed_methods:
            return PlainTextResponse("Method not allowed", status_code=HTTP_403_FORBIDDEN)
            ...
            for header in headers:
                if header not in self.allowed_headers:
                    return PlainTextResponse(
                        f"Header not allowed: {header}", status_code=HTTP_403_FORBIDDEN
                    )
...
def create_cors_middleware(
    allowed_origins: Optional[List[str]] = None, security_level: Optional[CORSSecurityLevel] = None
) -> Callable:
