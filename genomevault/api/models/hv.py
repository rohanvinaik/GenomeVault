...
class VectorInput(BaseModel):
    ...
    vector: List[float] = Field(
        ...,
        description="Numeric vector/array",
        min_items=100,
        max_items=100000
    )
...
class EncodingRequest(BaseModel):
    ...
    dimension: int = Field(
        default=10000,
        description="Target hypervector dimension",
        ge=1000,
        le=100000
    )
    ...
    def validate_input_type(cls, values):
        ...
        if variants is None and vector is None:
            raise ValueError(
                "Must provide either 'variants' or 'vector' input. "
...
class SearchRequest(BaseModel):
    ...
    query_vector: List[float] = Field(
        ...,
        description="Query hypervector for search",
        min_items=100,
        max_items=100000
    )
    ...
    def validate_query_vector(cls, v: List[float]) -> List[float]:
        ...
        for i, val in enumerate(v):
            ...
            if not (-1e10 < val < 1e10):
                raise ValueError(
                    f"Query vector element at index {i} is out of bounds: {val}"
                )
        return v
...
class SearchResponse(BaseModel):
    ...
    query_dimension: int = Field(
        ...,
        description="Dimension of the query vector"
    )
    ...
    metric: str = Field(
        ...,
        description="Distance metric used for search"
    )
