...
def parse_vcf_file(file_content: bytes) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Invalid VCF file format: {str(e)}"
        )


def calculate_similarity(hv1: np.ndarray, hv2: np.ndarray, metric: str = "hamming") -> tuple[float, float]:
    """
...
async def get_encoding(
    ...
    try:
        # Query database
        encoding = db.query(HDCEncoding).filter(
            HDCEncoding.encoding_id == encoding_id
        ).first()
        
        if not encoding:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Encoding {encoding_id} not found"
            )
...
async def compare_encodings(
    ...
    try:
        ...
        encoding1 = db.query(HDCEncoding).filter(
            HDCEncoding.encoding_id == request.encoding_id_1
        ).first()
        ...
        encoding2 = db.query(HDCEncoding).filter(
            HDCEncoding.encoding_id == request.encoding_id_2
        ).first()
...
async def batch_compare_encodings(
    ...
    try:
        ...
        base_encoding = db.query(HDCEncoding).filter(
            HDCEncoding.encoding_id == encoding_id
        ).first()
        ...
        for comp_id in comparison_ids:
            ...
            comp_encoding = db.query(HDCEncoding).filter(
                HDCEncoding.encoding_id == comp_id
            ).first()
