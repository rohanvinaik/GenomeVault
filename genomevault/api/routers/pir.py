...
async def setup_dataset(request: PIRSetupRequest):
    ...
    try:
        ...
        logger.info(f"PIR dataset initialized: {request.dataset_size} elements, type: {request.dataset_type}")
...
async def get_status():
    ...
    system_healthy = (
        _server_status.get("initialized", False) and
        healthy_servers >= 2  # Need at least 2 servers for PIR
    )
