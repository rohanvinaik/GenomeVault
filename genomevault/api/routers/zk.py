...
AVAILABLE_CIRCUITS = {
        ...
        "required_inputs": [
            {
                ...
                "type": "string",
                "visibility": "public",
                "description": "Hash of the genome",
            },
            {
                "name": "variant_position",
                "type": "integer",
                "visibility": "public",
                "description": "Position of variant",
            },
            {
                "name": "variant_allele",
                "type": "string",
    ...
    "ancestry_proof": {
        ...
        "required_inputs": [
            {
                "name": "ancestry_ranges",
                "type": "object",
                "visibility": "public",
                "description": "Acceptable ancestry ranges",
            },
            {
                "name": "ancestry_composition",
                "type": "object",
                "visibility": "private",
                "description": "Actual ancestry percentages",
            },
        ],
        "constraints": 25000,
        "supported": True,
        ...
        "required_inputs": [
            {
                ...
                "type": "string",
                "visibility": "public",
                "description": "Hash of eligibility criteria",
            },
            {
                "name": "eligible",
                "type": "boolean",
            ...
            },
            {
                ...
                "type": "object",
                "visibility": "private",
                "description": "Genomic test results",
            },
        ],
        "constraints": 100000,
        "supported": True,
    ...
    "computation_result": {
        ...
        "required_inputs": [
            ...
            {
                "name": "computation_hash",
                "type": "string",
                "visibility": "public",
                "description": "Hash of computation",
            },
            {
                "name": "inputs",
                "type": "array",
...
async def verify_proof(
    ...
    try:
        ...
        if request.proof_id and not circuit_name:
            record = (
                db.query(ZKProofRecord).filter(ZKProofRecord.proof_id == request.proof_id).first()
            )
            if record:
        ...
        if request.proof_id:
            try:
                record = (
                    ...
                    .filter(ZKProofRecord.proof_id == request.proof_id)
                    .first()
                )
                if record:
...
async def get_circuit_info(circuit_name: str):
    ...
    try:
        if circuit_name not in AVAILABLE_CIRCUITS:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=f"Circuit {circuit_name} not found"
            )
...
async def get_proof_info(proof_id: str, db: Session = Depends(get_db)):
    """
    ...
    """
    try:
        record = db.query(ZKProofRecord).filter(ZKProofRecord.proof_id == proof_id).first()

        if not record:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=f"Proof {proof_id} not found"
            )
        ...
        for i, proof_request in enumerate(proofs):
            ...
            try:
                ...
                if proof_request.proof_id and not circuit_name:
                    record = (
                        db.query(ZKProofRecord)
                        .filter(ZKProofRecord.proof_id == proof_request.proof_id)
                        .first()
                    )
                    if record:
                        circuit_name = record.circuit_name
