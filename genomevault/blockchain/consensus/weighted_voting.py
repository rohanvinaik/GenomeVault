...
class BFTConsensus:
    ...
    def submit_vote(self, node_id: str, block_hash: str, signature: str = "mock_sig") -> bool:
        ...
        logger.debug(
            f"Vote recorded: {node_id} (w={node.voting_weight}) -> {block_hash[:8]}..."
        )
    ...
    def check_consensus(self, round_num: Optional[int] = None) -> Optional[str]:
        ...
        logger.debug(
            f"No consensus yet. Max weight: {max_weight}/{self.threshold}"
        )
