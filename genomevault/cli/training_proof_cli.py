...
def verify_proof(proof_file: str, snapshot_dir: str, output: Optional[str], verbose: bool):
    ...
    if not snapshot_check:
        click.echo(f"❌ Snapshot count mismatch: expected {expected_snapshots}, got {actual_snapshots}")
    ...
    if snapshots:
        ...
        timeline_check = (
            abs(start_time - actual_start) < 60 and  # Within 1 minute
            abs(end_time - actual_end) < 60
        )
...
@training_proof_cli.command()
@click.option('--snapshot-dir', '-s', required=True, help='Directory containing model snapshots')
@click.option('--output-dir', '-o', default='./drift_analysis', help='Output directory for visualizations')
...
@training_proof_cli.command()
@click.option('--contract-address', '-c', required=True, help='Training attestation contract address')
...
def check_attestation(contract_address: str, attestation_id: str, chain_id: int, verify: bool):
    ...
    if verify:
        ...
        if verifications:
            ...
            click.echo(tabulate(
                table_data,
                headers=["Verifier", "Result", "Timestamp", "Notes"]
            ))
