...
active_breaches = Gauge(
    "genomevault_active_breaches", "Number of active breaches under investigation", ["severity"]
)
...
class BreachEvent:
    ...
    def is_major_breach(self) -> bool:
        ...
        return (
            self.estimated_records >= 500
            or self.severity in [BreachSeverity.HIGH, BreachSeverity.CRITICAL]
            or self.breach_type in [BreachType.DATA_EXFILTRATION, BreachType.RANSOMWARE]
        )
    ...
    def requires_immediate_notification(self) -> bool:
        ...
        return (
            ...
            or self.breach_type in [BreachType.RANSOMWARE, BreachType.DATA_EXFILTRATION]
            or self.estimated_records >= 1000
        )
...
This is an automated alert from GenomeVault Breach Detection System.
    ...
    def send_notification(
        ...
        **kwargs,
        ...
        try:
            ...
            elif notification_type in [NotificationType.HIPAA_HHS, NotificationType.GDPR_AUTHORITY]:
            ...
            if success:
                ...
                breach_notifications.labels(
                    notification_type=notification_type.value, channel="email"
                ).inc()

                logger.info(
                    f"Sent {notification_type.value} notification for breach {breach.event_id}"
                )
    ...
    def _get_template(
        self, notification_type: NotificationType, breach: BreachEvent, **kwargs
    ) -> Dict[str, str]:
        ...
        formatted = template.format(
            ...
            remediation_steps=kwargs.get(
                "remediation_steps", "Investigation and remediation in progress"
            ),
    ...
    def _send_regulatory_notification(
        self, notification_type: NotificationType, template: Dict[str, str], **kwargs
    ) -> bool:
...
class BreachNotificationSystem:
    ...
    def _send_immediate_alerts(self, breach: BreachEvent) -> None:
        ...
        self.notification_manager.send_notification(
            NotificationType.INTERNAL_ALERT, breach, recipients
        )
    ...
    def _send_required_notifications(
        self, breach: BreachEvent, risk_assessment: Dict[str, Any]
    ) -> None:
        ...
        if breach.phi_involved:
            ...
            if risk_assessment["risk_level"] in ["HIGH", "MEDIUM"]:
                ...
                self.notification_manager.send_notification(
                    NotificationType.GDPR_INDIVIDUAL, breach, individual_emails
                )
        ...
        if breach.phi_involved and risk_assessment["notification_required"]:
            # HHS notification
            self.notification_manager.send_notification(
                NotificationType.HIPAA_HHS, breach, ["ocr@hhs.gov"]
            )

            # Individual notifications
            individual_emails = self._get_individual_contact_info(breach.affected_individuals)
            self.notification_manager.send_notification(
                NotificationType.HIPAA_INDIVIDUAL, breach, individual_emails
            )

            # Media notification if major breach (500+)
            if breach.is_major_breach():
                self.notification_manager.send_notification(
                    NotificationType.HIPAA_MEDIA, breach, ["media@genomevault.io"]
                )
