...
# Metrics
cache_hits = Counter(
    "genomevault_cache_hits_total", "Total number of cache hits", ["cache_type", "operation"]
)

cache_misses = Counter(
    "genomevault_cache_misses_total", "Total number of cache misses", ["cache_type", "operation"]
)

cache_evictions = Counter(
    "genomevault_cache_evictions_total", "Total number of cache evictions", ["cache_type", "reason"]
)

cache_size = Gauge("genomevault_cache_size_bytes", "Current cache size in bytes", ["cache_type"])
...
class HypervectorCache:
    ...
    def get_encoding(self, variant_data: Dict[str, Any]) -> Optional[np.ndarray]:
        ...
        with cache_operation_duration.labels(cache_type="hypervector", operation="get").time():
    ...
    def put_encoding(self, variant_data: Dict[str, Any], encoding: np.ndarray) -> None:
        ...
        with cache_operation_duration.labels(cache_type="hypervector", operation="put").time():
    ...
    def _acquire(self) -> Any:
        ...
        with self.lock:
            ...
            self.available_connections = [
                (conn, last_used)
                for conn, last_used in self.available_connections
...
class CacheManager:
    ...
    def get_connection_pool(
        self, pool_name: str, create_connection: Callable, max_connections: int = 20
    ) -> ConnectionPool:
        """Get or create connection pool."""
        if pool_name not in self.connection_pools:
            self.connection_pools[pool_name] = ConnectionPool(
                create_connection=create_connection, max_connections=max_connections
            )
