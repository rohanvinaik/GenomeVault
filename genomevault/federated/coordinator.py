...
class DifferentialPrivacyMechanism:
    ...
    def add_noise(self, gradient: np.ndarray, sensitivity: float) -> Tuple[np.ndarray, float]:
        ...
        """
        # Calibrate noise
        noise_std = self.calibrate_noise(sensitivity, self.params.epsilon, self.params.delta)
    ...
    def generate_noise_proof(self, noise_std: float, gradient_norm: float) -> Dict[str, Any]:
        ...
        proof = {
            "commitment": hashlib.sha256(f"{noise_std}:{gradient_norm}".encode()).hexdigest(),
            "noise_std": noise_std,
        ...
        proof["signature"] = hashlib.sha256(json.dumps(proof, sort_keys=True).encode()).hexdigest()
...
class PrivacyAccountant:
    ...
    def can_proceed(self, epsilon: float, delta: float) -> bool:
        ...
        return (
            ...
            and self.consumed_delta + delta <= self.total_delta
        )
    ...
    def get_remaining_budget(self) -> Tuple[float, float]:
        ...
        return (self.total_epsilon - self.consumed_epsilon, self.total_delta - self.consumed_delta)
...
class ConvergenceDetector:
    ...
    def has_converged(self) -> bool:
        ...
        return (
            loss_std < self.threshold
            and loss_change < self.threshold
            and avg_grad_norm < self.threshold * 10
        )
...
class FederatedCoordinator:
    ...
    def __init__(
        ...
        self.ckks_context = (
            CKKSContext() if config.protocol == AggregationProtocol.HOMOMORPHIC else None
        )
    ...
    def select_participants(self, round_num: int) -> List[str]:
        ...
        """
        active_participants = [
            p for p in self.participants.values() if p.status == ParticipantStatus.ACTIVE
        ]
        ...
        selected_indices = np.random.choice(
            len(active_participants), size=num_select, replace=False, p=probabilities
        )
    ...
    def local_update(
        ...
        for layer_name, weights in global_weights.items():
            ...
            noisy_gradient, noise_std = self.dp_mechanism.add_noise(
                clipped_gradient, self.privacy_params.sensitivity
            )
    ...
    def secure_aggregate(
        self, updates: List[Dict[str, np.ndarray]], participants: List[str]
    ) -> Dict[str, np.ndarray]:
        """
    ...
    def _homomorphic_aggregation(
        self, updates: List[Dict[str, np.ndarray]]
    ) -> Dict[str, np.ndarray]:
        ...
        for layer_name in updates[0].keys():
            ...
            for encrypted in encrypted_updates[1:]:
                encrypted_sum = self.ckks_context.add_encrypted(encrypted_sum, encrypted)
    ...
    def _secure_agg_malicious(
        ...
        vector_size = sum(w.size for w in updates[0].values())
    ...
    def _plain_aggregation(self, updates: List[Dict[str, np.ndarray]]) -> Dict[str, np.ndarray]:
    ...
    def train_round(
        self, round_num: int, train_data: Optional[Dict[str, Tuple[np.ndarray, np.ndarray]]] = None
    ) -> Dict[str, Any]:
        """
        ...
        if not self.privacy_accountant.can_proceed(
            self.privacy_params.epsilon, self.privacy_params.delta
        ):
        ...
        for p_id in selected:
            ...
            local_weights, metadata = self.local_update(
                p_id, self.global_model or self._initialize_model(), data, labels
            )
        ...
        self.privacy_accountant.consume_budget(
            round_num, self.privacy_params.epsilon, self.privacy_params.delta
        )
    ...
    def evaluate_model(self, test_data: np.ndarray, test_labels: np.ndarray) -> Dict[str, float]:
        """
        ...
        return {"accuracy": accuracy, "loss": loss, "round": self.current_round}
    ...
    def generate_training_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "participants": {
                ...
                "active": sum(
                    1 for p in self.participants.values() if p.status == ParticipantStatus.ACTIVE
                ),
                "dropped": sum(
                    1 for p in self.participants.values() if p.status == ParticipantStatus.DROPPED
                ),
            },
            "convergence": {
                "converged": self.convergence_detector.has_converged(),
            ...
            "final_loss": (
                self.training_history[-1]["global_loss"] if self.training_history else None
            ),
...
def create_coordinator(
    ...
    """
    config = TrainingConfig(
        num_rounds=num_rounds, protocol=protocol, min_participants=5, participation_rate=0.3
    )

    privacy_params = PrivacyParameters(epsilon=epsilon, delta=1e-5, clip_norm=1.0)
