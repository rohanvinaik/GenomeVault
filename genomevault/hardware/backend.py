    ...
    try:
        ...
        if torch.cuda.is_available():
            return HardwareBackend(
                ...
                preferred_precision=(
                    "float16" if torch.cuda.get_device_capability(0)[0] >= 7 else "float32"
                ),
            )
    except ImportError:
        pass
...
def get_best_accelerator(
    preferred_type: Optional[AcceleratorType] = None, min_memory_gb: Optional[float] = None
) -> HardwareBackend:
    """
...
def get_accelerator_info() -> Dict[str, Any]:
    ...
    info = {"available_count": sum(1 for a in accelerators if a.available), "accelerators": []}
