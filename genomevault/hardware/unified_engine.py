...
class UnifiedAccelerationEngine:
    ...
    def _init_cuda(self):
        ...
        try:
            ...
            mempool.set_limit(size=int(cp.cuda.Device().mem_info[1] * self.config.memory_fraction))
    ...
    def _init_cpu(self):
        ...
        self.backend = HardwareBackend(type=AcceleratorType.CPU, name="CPU", available=True)
    ...
    def random_projection_matrix(
        self, input_dim: int, output_dim: int, sparse: bool = False
    ) -> Any:
        """
        ...
        if sparse:
            ...
            if self.backend.type == AcceleratorType.METAL and hasattr(self, "mx"):
                return self.mx.random.normal(
                    shape=[output_dim, input_dim], dtype=self.dtype
                ) / np.sqrt(input_dim)
            elif self.backend.type == AcceleratorType.CUDA and hasattr(self, "cp"):
                return self.cp.random.normal(
                    size=(output_dim, input_dim), dtype=self.cp.float32
                ) / np.sqrt(input_dim)
            else:
