#!/usr/bin/env python3
"""
Discover Safe Correction Signatures for int8

Apply the same conservative signature discovery methodology to int8 quantization.
Expected to yield similar biophysical patterns as float32.
"""
...
def test_signature(
    ...
    """Test a signature's performance on int8 data."""

    int8_thresholds = {'AT': 0.05, 'GC': 0.00, 'PuPy': 0.20, 'AmKe': 0.20, 'StWk': 0.20}
    ...
    for tm in teachable_moments:
        lens_results = tm['lens_results']['int8']
        confidence = tm['confidences']['int8']
        ...
        for lens, sim in lens_results.items():
            voted_base = vote_for_base_with_threshold(lens, sim, int8_thresholds[lens])
            if voted_base:
...
def discover_safe_signature_for_transform(
    ...
    """
    Discover the safest signature for a transformation (int8 version).
    """
    ...
    for lens in ['AT', 'GC', 'PuPy', 'AmKe', 'StWk']:
        mags = [abs(fix['lens_results']['int8'][lens]) for fix in successful_fixes]
...
def main():
    logger.info("="*80)
    logger.info("DISCOVER SAFE CORRECTION SIGNATURES (INT8)")
    logger.info("="*80)
    logger.info("")
    ...
    int8_file = Path("HDV_VALIDATION_PACKAGE/architecture_testing/aligned_10k/comparison_results/int8_predictions_detailed.json")
    with open(int8_file, 'r') as f:
        all_int8 = json.load(f)
    correct_predictions = [p for p in all_int8 if p['correct']]
    ...
    # Define transformations to test (same as float32)
    transforms = [
        # Previously successful in float32
        {
    ...
    logger.info("")
    logger.info("="*80)
    logger.info("SAFE SIGNATURES DISCOVERED (INT8)")
    logger.info("="*80)
