...
class OrthogonalProjection:
    ...
    def __init__(self, n_components: int, seed: Optional[int] = None) -> None:
        if not isinstance(n_components, int) or n_components <= 0:
            raise ProjectionError("n_components must be a positive integer", context={"n_components": n_components})
        self.n_components = int(n_components)
        self.rng = np.random.default_rng(seed)
        self._P: Optional[np.ndarray] = None  # shape: (n_components, n_features)
    ...
    def fit(self, n_features: int) -> "OrthogonalProjection":
        ...
        if n_features < self.n_components:
            raise ProjectionError("n_features must be >= n_components", context={"n_features": n_features, "n_components": self.n_components})
        # Gaussian random matrix, then QR
        A = self.rng.standard_normal((n_features, self.n_components))  # (n_features, n_components)
        Q, _ = np.linalg.qr(A, mode="reduced")  # Q: (n_features, n_components) with orthonormal columns
