...
class ModelEvolutionVisualizer:
    ...
    def visualize_semantic_space(
        ...
        reducer = umap.UMAP(
            n_neighbors=min(15, len(hypervectors) - 1),
            min_dist=0.1,
            random_state=42
        )
    ...
    def plot_drift_analysis(
        ...
        if anomaly_indices:
            ...
            ax1.scatter(anomaly_indices, anomaly_scores, color='red', s=100, 
                       marker='x', linewidth=3, label='Anomalies')
        ...
        for idx, (start, end) in enumerate(phases):
            ...
            for i in range(1, len(phase_embeddings)):
                plt.plot(
                    ...
                    color=phase_colors[idx], alpha=0.5, linewidth=2
                )
...
def create_semantic_debugging_report(
    ...
    for i in range(1, len(embeddings)):
        ax1.plot([embeddings[i-1, 0], embeddings[i, 0]],
                [embeddings[i-1, 1], embeddings[i, 1]], 'k-', alpha=0.3)
    ...
    for idx, (start, end) in enumerate(phases):
        ax3.axvspan(start, end, alpha=0.3, color=phase_colors[idx], 
                   label=f'Phase {idx+1}')
