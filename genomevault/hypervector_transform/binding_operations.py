...
class HypervectorBinder:
    ...
    def bind(self, vectors: list[torch.Tensor], binding_type: BindingType = BindingType.MULTIPLY) -> torch.Tensor:
        if not vectors:
    ...
    def unbind(self, bound_vector: torch.Tensor, known_vectors: list[torch.Tensor], binding_type: BindingType = BindingType.MULTIPLY) -> torch.Tensor:
    ...
    def create_composite_binding(self, role_filler_pairs: list[tuple[torch.Tensor, torch.Tensor]], binding_type: BindingType = BindingType.MULTIPLY) -> torch.Tensor:
        bound_pairs = []
