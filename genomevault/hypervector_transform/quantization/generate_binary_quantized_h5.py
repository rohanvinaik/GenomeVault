#!/usr/bin/env python3
"""
Generate binary quantized version of 3D HDF5 file.

Simple sign-based quantization: {-1, 0, +1} stored as int8.
...
def quantize_float32_to_binary(data: np.ndarray) -> np.ndarray:
    """
    Quantize float32 data to binary {-1, 0, +1}.
    ...
    """
    # Simple sign operation: -1 for negative, 0 for zero, +1 for positive
    return np.sign(data).astype(np.int8)


def generate_binary_h5(input_path: Path, output_path: Path, batch_size: int = 10000):
    """
    Generate binary quantized HDF5 file from float32 version.

    Args:
        input_path: Path to input float32 3D HDF5
        output_path: Path to output binary 3D HDF5
        batch_size: Number of chunks to process at once (default: 10000)
    """
    logger.info("=" * 80)
    logger.info("BINARY QUANTIZATION - 3D HDF5")
    logger.info("=" * 80)
    logger.info("")
    ...
    with h5py.File(input_path, 'r') as f_in:
        ...
        logger.info(f"  Note: Binary stored as int8 (3 values: -1, 0, +1)")
        logger.info(f"  Note: No compression - gzip would EXPAND the file")
        logger.info("")

        # Create output file
        logger.info("Creating output file (UNCOMPRESSED)...")
        with h5py.File(output_path, 'w') as f_out:
            # Create dataset with same shape but int8 dtype
            # NO COMPRESSION - gzip overhead would expand binary data
            ...
            chunks_out.attrs['quantization'] = 'binary'
            chunks_out.attrs['values'] = '{-1, 0, +1}'
            ...
            for batch_idx in range(num_batches):
                ...
                # Quantize (simple sign operation)
                quantized_batch = quantize_float32_to_binary(batch_data)
    ...
    logger.info("")
    logger.info("✓ Binary quantization complete!")
    logger.info("")


if __name__ == '__main__':
    input_path = Path("data/experimental_strands/ERR3239334/hdv_encoding/encoded_genome_5lenses_3d.h5")
    output_path = Path("data/experimental_strands/ERR3239334/hdv_encoding/encoded_genome_5lenses_3d_binary.h5")
    ...
    generate_binary_h5(input_path, output_path, batch_size=10000)
