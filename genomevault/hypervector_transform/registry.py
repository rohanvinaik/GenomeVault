...
class HypervectorRegistry:
    ...
    def register_version(self, version: str, params: Dict[str, Any], 
    ...
    def check_compatibility(self, version1: str, version2: str) -> Dict[str, Any]:
        ...
        if key1 in self.compatibility_map:
            ...
            compatible = (
                params1["dimension"] == params2["dimension"] and
                params1["projection_type"] == params2["projection_type"]
            )
...
class VersionMigrator:
    ...
    def migrate_hypervector(self, hv: np.ndarray, 
                           from_version: str, 
        ...
        if from_params["dimension"] != to_params["dimension"]:
            if from_params["dimension"] > to_params["dimension"]:
                return self._migrate_dimension_reduction(
                    hv, from_params, to_params
                )
            else:
                return self._migrate_dimension_expansion(
                    hv, from_params, to_params
                )
        
        elif from_params["projection_type"] != to_params["projection_type"]:
            return self._migrate_projection_change(
                hv, from_params, to_params
            )
    ...
    def _migrate_dimension_reduction(self, hv: np.ndarray,
                                   from_params: Dict[str, Any],
                                   to_params: Dict[str, Any]) -> np.ndarray:
    ...
    def _migrate_dimension_expansion(self, hv: np.ndarray,
                                   from_params: Dict[str, Any],
                                   to_params: Dict[str, Any]) -> np.ndarray:
    ...
    def _migrate_projection_change(self, hv: np.ndarray,
                                 from_params: Dict[str, Any],
                                 to_params: Dict[str, Any]) -> np.ndarray:
    ...
    def _migrate_re_encode(self, hv: np.ndarray,
                         from_params: Dict[str, Any],
                         to_params: Dict[str, Any]) -> np.ndarray:
        ...
        logger.warning(
            "Re-encoding migration is lossy - "
            "original data needed for perfect migration"
        )
