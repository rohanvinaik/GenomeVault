...
class RealTimeModelMonitor:
    ...
    def __init__(
        ...
        self.prediction_window: Deque = deque(
    ...
    def get_monitoring_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "predictions_per_hour": round(
                self.state.total_predictions / uptime_hours, 2
    ...
    def trigger_retraining_protocol(self) -> Dict[str, Any]:
        ...
        retraining_request = {
            ...
            "model_id": self.model_id,
            "timestamp": int(time.time()),
            "urgency": "high" if DriftSeverity.CRITICAL in [
                e.severity for e in critical_drifts
            ] else "medium",
            "drift_summary": {
            ...
            "recommended_approach": self._recommend_retraining_approach(
                drift_types
            ),
...
class PredictionDriftDetector:
    ...
    def detect_drift(
        ...
        if "quantiles" in baseline_pred_stats:
            quantile_diffs = [
                abs(c - b) for c, b in zip(
                    current_stats["quantiles"],
                    baseline_pred_stats["quantiles"]
                )
            ]
