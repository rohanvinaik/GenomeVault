...
class GenomicsPipeline(BasePipeline):
    ...
    async def run(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.add_audit_entry("pipeline_started", hashlib.sha256(str(input_data).encode()).hexdigest())
        
        try:
            # Step 1: Alignment with BWA-MEM2
            logger.info("Starting alignment with BWA-MEM2")
            bam_file = await self._run_bwa_alignment(
                input_data["fastq_r1"],
                input_data["fastq_r2"]
            )
            ...
            self.add_audit_entry("pipeline_completed", hashlib.sha256(filtered_vcf.encode()).hexdigest())
    ...
    async def _run_bwa_alignment(self, fastq_r1: str, fastq_r2: str) -> str:
        ...
        if self.runtime == ContainerRuntime.DOCKER:
            ...
            if result["StatusCode"] != 0:
                raise ContainerError(container, result["StatusCode"], "bwa-mem2", self.containers["bwa"], logs)
        else:
    ...
    async def run(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            abundance_file = await self._run_kallisto(
                ...
                input_data["transcriptome_index"]
            )
            ...
            self.status = PipelineStatus.COMPLETED
            self.add_audit_entry("pipeline_completed", hashlib.sha256(count_matrix.encode()).hexdigest())
...
class EpigenomicsPipeline(BasePipeline):
    ...
    async def run(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.status = PipelineStatus.COMPLETED
            self.add_audit_entry("pipeline_completed", hashlib.sha256(dmr_file.encode()).hexdigest())
...
class ClinicalPipeline(BasePipeline):
    ...
    async def run(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.status = PipelineStatus.COMPLETED
            self.add_audit_entry("pipeline_completed", hashlib.sha256(json.dumps(features).encode()).hexdigest())
            ...
            def __exit__(self, exc_type, exc_val, exc_tb):
                ...
                if self.pipeline_id in self.monitor.active_monitors:
                    self.monitor.active_monitors[self.pipeline_id].update({
                        ...
                        "peak_cpu": max(self.start_resources["cpu_percent"], end_resources["cpu_percent"]),
                        "peak_memory": max(self.start_resources["memory_percent"], end_resources["memory_percent"])
                    })
                
                logger.info(f"Pipeline {self.pipeline_id} resource usage: "
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        pipeline = await manager.create_pipeline(
            "genomics",
            resources=ResourceRequirements.from_profile(ResourceProfile.STANDARD)
        )
