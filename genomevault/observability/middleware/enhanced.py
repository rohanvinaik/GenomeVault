...
class EnhancedObservabilityMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        ...
        endpoint_path = getattr(route, "path_format", request.url.path) if route else request.url.path
        ...
        if self.tracing_manager:
            with self.tracing_manager.trace_operation(
                ...
                return await self._process_request(
                    request, call_next, request_id, user_id, client_ip,
                    endpoint_path, start_time, span
                )
        else:
            return await self._process_request(
                request, call_next, request_id, user_id, client_ip,
                endpoint_path, start_time, None
            )
    ...
    def _determine_privacy_level(self, path: str) -> str:
        ...
        if any(pattern in path.lower() for pattern in [
            '/zk/', '/pir/', '/clinical/', '/variants/', '/genomic/'
        ]):
            return "high"
        
        # Medium privacy endpoints
        if any(pattern in path.lower() for pattern in [
            '/hdc/', '/encode/', '/federated/'
        ]):
            return "medium"
