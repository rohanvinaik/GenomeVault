...
class TracingManager:
    ...
    def span(self, 
        ...
        with self.tracer.start_as_current_span(
            name=name,
            kind=kind,
            attributes=self._prepare_attributes(span_type, attributes)
        ) as span:
            try:
                yield span
    ...
    def _is_phi_attribute(self, key: str) -> bool:
        ...
        phi_keywords = [
            ...
            'genomic_data', 'variant', 'diagnosis', 'medication'
        ]
        
        key_lower = key.lower()
        return any(keyword in key_lower for keyword in phi_keywords)
    
    def trace_pir_query(self, 
                       server_id: int,
                       query_id: str,
                       query_type: str) -> Any:
        """
    ...
    def trace_pir_response(self,
                          server_id: int,
                          query_id: str,
                          response_size: int) -> Any:
        """
        Trace PIR response.
    ...
    def add_event(self, 
        ...
        if span and span.is_recording():
            span.add_event(
                name=name,
                attributes=self._prepare_attributes(SpanType.API, attributes)
            )
...
def trace_function(span_type: SpanType = SpanType.API,
...
def trace_async_function(span_type: SpanType = SpanType.API,
                        name: Optional[str] = None):
    """
