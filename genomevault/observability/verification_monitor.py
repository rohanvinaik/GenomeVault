...
class ZKVerificationMonitor:
    ...
    def start_proof_generation(self, 
        ...
        with self.lock:
            ...
            # Update gauge
            active_count = sum(
                1 for p in self.active_proofs.values() 
    ...
    def end_proof_generation(self, 
        ...
        with self.lock:
            ...
            self.generation_time.labels(
                type=metrics.proof_type.value
            ).observe(generation_time)
            
            self.proof_size.labels(
                type=metrics.proof_type.value
            ).observe(len(proof_data))
        ...
        with self.lock:
            ...
            try:
                ...
                if verification_successful:
                    ...
                    # Update metrics
                    self.verification_time.labels(
                        type=metrics.proof_type.value
                    ).observe(verification_time)
                    
                    self.proof_total.labels(
                        type=metrics.proof_type.value,
                        status='verified'
                    ).inc()
                ...
                active_count = sum(
                    1 for p in self.active_proofs.values() 
                    if p.proof_type == metrics.proof_type
                )
                self.active_proofs_gauge.labels(
                    type=metrics.proof_type.value
                ).set(active_count)
    ...
    def _update_statistics(self, metrics: ProofMetrics) -> None:
        ...
        stats.avg_generation_time_ms = (
            alpha * metrics.generation_time_ms + 
            (1 - alpha) * stats.avg_generation_time_ms
        )
        
        stats.avg_verification_time_ms = (
            alpha * metrics.verification_time_ms + 
            (1 - alpha) * stats.avg_verification_time_ms
        )
        
        stats.avg_proof_size_bytes = (
            alpha * metrics.proof_size_bytes + 
            (1 - alpha) * stats.avg_proof_size_bytes
        )
        ...
        self.verification_rate_gauge.labels(
            type=metrics.proof_type.value
        ).set(stats.verification_rate)
    ...
    def get_statistics(self, 
                       proof_type: Optional[ProofType] = None) -> Dict[str, Any]:
        """
        ...
        with self.lock:
            if proof_type:
                ...
                all_stats = {
                    'by_type': {
                        pt.value: asdict(self.stats_by_type[pt])
                        for pt in ProofType
                    },
    ...
    def get_proof_history(self, 
                         limit: int = 100,
                         proof_type: Optional[ProofType] = None) -> List[Dict]:
        """
    ...
    def get_time_series_metrics(self, 
                               period: str = 'hour',
        ...
        with self.lock:
            ...
            for i in range(lookback):
                ...
                if key in metrics_dict:
                    ...
                    verified = sum(1 for m in metrics_list if m.status == VerificationStatus.VERIFIED)
                    failed = sum(1 for m in metrics_list if m.status == VerificationStatus.FAILED)
                    avg_gen_time = np.mean([m.generation_time_ms for m in metrics_list]) if metrics_list else 0
                    avg_ver_time = np.mean([m.verification_time_ms for m in metrics_list]) if metrics_list else 0
                    ...
                    time_series.append({
                        ...
                        'verified': verified,
                        'failed': failed,
                        'verification_rate': verified / len(metrics_list) if metrics_list else 0,
