...
class ByzantineHandler:
    ...
    def decode_responses(self, 
                        ...
                        expected_size: int) -> Optional[bytes]:
        """
    ...
    def detect_byzantine_behavior(self, 
                                 responses: List[Tuple[int, bytes]]) -> List[int]:
        """
    ...
    def verify_response_consistency(self, 
    ...
    def select_reliable_servers(self, num_needed: int) -> List[int]:
        ...
        sorted_servers = sorted(
            self.server_reputations.values(),
        ...
        reliable_servers = [
            s.server_id for s in sorted_servers
            if s.reliability_score >= self.config.reputation_threshold
        ]
        
        if len(reliable_servers) < num_needed:
            logger.warning(
                f"Only {len(reliable_servers)} reliable servers available, "
                f"need {num_needed}"
            )
...
class AdaptiveByzantineHandler(ByzantineHandler):
    ...
    def adapt_strategy(self) -> None:
        ...
        avg_reliability = np.mean([
            ...
            for rep in self.server_reputations.values()
        ])
    ...
    def fast_path_decode(self, responses: List[Tuple[int, bytes]]) -> Optional[bytes]:
        ...
        if len(responses) >= self.config.min_servers:
            ...
            reliable_responses = [
                ...
                if self.server_reputations[r[0]].reliability_score > 0.9
            ]
