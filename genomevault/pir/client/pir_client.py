...
class PIRClient:
    ...
    def __init__(
        ...
        if len(servers) < min_servers:
            raise ValueError(f"Need at least {min_servers} servers, got {len(servers)}")
    ...
    def generate_it_pir_query(self, index: int) -> PIRQuery:
        ...
        return PIRQuery(index=index, query_vectors=query_vectors, protocol=QueryProtocol.IT_PIR)
    ...
    def generate_xor_query(self, index: int) -> PIRQuery:
        ...
        return PIRQuery(index=index, query_vectors=[mask1, mask2], protocol=QueryProtocol.XOR)

    async def query_server(
        self, server: ServerConfig, query_vector: np.ndarray, query_id: str, retry_count: int = 0
    ) -> PIRResponse:
        """
        ...
        try:
            ...
            async with self.session.post(
                f"{server.url}/query", json=payload, timeout=timeout
            ) as response:
            ...
            # Retry on timeout
            if retry_count < server.max_retries:
                return await self.query_server(server, query_vector, query_id, retry_count + 1)
    ...
    def aggregate_responses_xor(self, responses: List[PIRResponse]) -> bytes:
        ...
        for response in responses[1:]:
            if len(response.data) != len(result):
                raise ValueError(f"Response size mismatch: {len(response.data)} != {len(result)}")
    ...
    def validate_response_integrity(
        self, response: bytes, expected_size: int, checksum: Optional[str] = None
    ) -> bool:
        """
        ...
        if len(response) != expected_size:
            logger.warning(f"Response size mismatch: {len(response)} != {expected_size}")
            return False
    ...
    async def retrieve(self, index: int, protocol: Optional[QueryProtocol] = None) -> bytes:
        """
        ...
        for i, server in enumerate(self.servers):
            if i < len(query.query_vectors):
                task = self.query_server(server, query.query_vectors[i], query.query_id)
                tasks.append(task)
    ...
    async def batch_retrieve(self, indices: List[int], max_concurrent: int = 10) -> List[bytes]:
        """
        ...
        for server in self.servers:
            try:
                async with self.session.get(
                    f"{server.url}/status", timeout=aiohttp.ClientTimeout(total=5.0)
                ) as response:
                    if response.status == 200:
                        status = await response.json()
                        statuses[server.server_id] = {"online": True, "status": status}
                    else:
                ...
                statuses[server.server_id] = {"online": False, "error": str(e)}
