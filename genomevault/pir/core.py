...
class PIRServer:
    ...
    def __init__(self, database: List[bytes], config: PIRConfig):
        ...
        if len(database) != config.database_size:
            raise ValueError(f"Database size mismatch: expected {config.database_size}, got {len(database)}")
...
def create_pir_system(database: List[bytes], 
                     security_parameter: int = 128) -> Tuple[PIRClient, PIRServer]:
