...
class Server:
    ...
    def is_available(self) -> bool:
        ...
        return self.status == ServerStatus.AVAILABLE and self.current_load < self.capacity
...
class TopologyManager:
    ...
    def __init__(self, config: Optional[TopologyConfig] = None):
        ...
        logger.info(
            "Initialized TopologyManager with target failure probability: %.2e",
    ...
    def calculate_optimal_shards(self, database_size: int, server_reliability: float = 0.99) -> int:
        """
    ...
    def select_servers(self, num_shards: int, prefer_trusted: bool = True) -> List[Server]:
        """
        ...
        # Separate by type
        trusted_servers = [s for s in available_servers if s.type == ServerType.TRUSTED_SIGNATORY]
        light_nodes = [s for s in available_servers if s.type == ServerType.LIGHT_NODE]
        full_nodes = [s for s in available_servers if s.type == ServerType.FULL_NODE]
    ...
    def calculate_failure_probability(self, servers: List[Server], threshold: int) -> float:
        """
    ...
    def get_configuration(
        self, database_size: int, use_preset: Optional[str] = None
        ...
        logger.info(
            f"Selected configuration: {config.total_shards} shards, "
    ...
    def should_reconfigure(self) -> bool:
        ...
        if recent_latencies:
            ...
            if avg_latency > threshold:
                logger.warning(
                    f"Latency {avg_latency:.1f}ms exceeds threshold {threshold:.1f}ms, reconfiguring"
                )
                return True

        # Check server failures
        failed_servers = sum(1 for s in self.servers.values() if s.status == ServerStatus.FAILED)
    ...
    def get_stats(self) -> Dict:
        ...
        failed = sum(1 for s in self.servers.values() if s.status == ServerStatus.FAILED)
...
class AdaptiveTopologyManager(TopologyManager):
    ...
    def adaptive_select(self, database_size: int) -> Tuple[ShardConfiguration, List[Server]]:
        """
    ...
    def record_performance(self, config_name: str, latency_ms: float, success: bool) -> None:
        """
