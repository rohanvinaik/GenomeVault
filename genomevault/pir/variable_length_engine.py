...
class VariableLengthPIREngine:
    ...
    def prepare_database(self, records: List[Union[str, bytes, dict, Any]]) -> Tuple[np.ndarray, List[int]]:
        """
    ...
    def get_stats(self, database: np.ndarray, original_lengths: List[int]) -> dict:
        ...
        stats = {
            ...
            'padding_efficiency': {
                ...
                'overhead_ratio': (database.size - sum(original_lengths)) / sum(original_lengths) if sum(original_lengths) > 0 else 0
            },
