...
class PHILeakageDetector:
    ...
    def scan_directory(self, directory: str, 
                      ...
                      exclude_patterns: List[str] = None) -> Dict[str, List[Dict[str, Any]]]:
        """
    ...
    def _scan_line(self, line: str, line_num: int, 
                   filename: str, max_context: int) -> List[Dict[str, Any]]:
    ...
    def generate_report(self, findings: List[Dict[str, Any]], 
        ...
        if output_format == "json":
            ...
            for severity in ["critical", "high", "medium", "low"]:
                if severity in by_severity:
                    html.append(f'<h2 class="{severity}">{severity.upper()} ({len(by_severity[severity])})</h2>')
                    html.append('<ul>')
                    
                    for finding in by_severity[severity][:10]:
                        html.append('<li>')
                        html.append(f'<strong>{finding["description"]}</strong> in {finding["file"]}<br>')
...
class RealTimePHIMonitor:
    ...
    def _trigger_alert(self):
        ...
        """
        logger.critical(
            f"PHI LEAKAGE ALERT: {len(self.findings_buffer)} instances detected!"
        )
