...
class SecretsManager:
    ...
    async def rotate_secret(
        self, secret_name: str, secret_type: SecretType, force: bool = False
        ...
        new_metadata = await self._store_secret(
            secret_name, new_secret, secret_type, metadata.version + 1
        )
        ...
        # Send notifications
        await self._send_notifications(secret_name, policy.notification_channels, "rotated")
    ...
    async def _store_secret(
        self, name: str, value: str, secret_type: SecretType, version: int
    ) -> SecretMetadata:
    ...
    async def _store_k8s_secret(self, name: str, value: str, metadata: SecretMetadata):
        ...
        try:
            ...
            self.k8s_client.patch_namespaced_secret(
                name=name, namespace=self.namespace, body=k8s_secret
            )
        except ApiException as e:
            if e.status == 404:
                # Create new secret if it doesn't exist
                self.k8s_client.create_namespaced_secret(namespace=self.namespace, body=k8s_secret)
            else:
                raise

    async def _store_aws_secret(self, name: str, value: str, metadata: SecretMetadata):
        ...
        if metadata.type == SecretType.PHI_ENCRYPTION_KEY:
            ...
            if kms_key_id and self.aws_kms_client:
                encrypted_value = self.aws_kms_client.encrypt(
                    KeyId=kms_key_id, Plaintext=value.encode()
                )
    ...
    async def _store_gcp_secret(self, name: str, value: str, metadata: SecretMetadata):
        ...
        if metadata.type == SecretType.PHI_ENCRYPTION_KEY:
            ...
            if kms_key_name and self.gcp_kms_client:
                encrypted_response = self.gcp_kms_client.encrypt(
                    request={"name": kms_key_name, "plaintext": value.encode()}
                )
    ...
    async def _store_vault_secret(self, name: str, value: str, metadata: SecretMetadata):
        ...
        if metadata.type == SecretType.PHI_ENCRYPTION_KEY:
            ...
            encrypted_response = self.vault_client.secrets.transit.encrypt_data(
                name=transit_key, plaintext=base64.b64encode(value.encode()).decode()
            )
    ...
    async def _get_k8s_secret(self, name: str) -> str:
        ...
        try:
            secret = self.k8s_client.read_namespaced_secret(name=name, namespace=self.namespace)
    ...
    async def _backup_secret(self, name: str, metadata: SecretMetadata):
        ...
        await self._store_secret(backup_name, current_value, metadata.type, metadata.version)
    ...
    async def _send_notifications(self, secret_name: str, channels: List[str], action: str):
    ...
    def encrypt_phi_data(self, data: str) -> str:
        ...
        payload = json.dumps({"data": data, "timestamp": timestamp, "nonce": nonce})
        ...
        for secret_type in SecretType:
            ...
            try:
                new_secret, metadata = await self.rotate_secret(
                    secret_name, secret_type, force=force
                )
                results[secret_name] = {
                ...
                results[secret_name] = {"status": "failed", "error": str(e)}
...
async def main():
    ...
    manager = SecretsManager(
        namespace=args.namespace, provider=SecretProvider[args.provider.upper()]
    )
    ...
    if args.action == "rotate":
        if args.secret_name and args.secret_type:
            ...
            new_secret, metadata = await manager.rotate_secret(
                args.secret_name, secret_type, force=args.force
            )
