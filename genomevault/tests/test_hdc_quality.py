...
class TestHDCQuality:
    ...
    def test_variant_similarity_preservation(self, encoder):
        ...
        variant_pairs = [
            ...
            ({"chr": "1", "pos": 12345, "ref": "A", "alt": "G"},
             {"chr": "1", "pos": 12345, "ref": "A", "alt": "T"}, 1),  # Same position, different alt
            
            ({"chr": "1", "pos": 12345, "ref": "A", "alt": "G"},
             {"chr": "1", "pos": 12346, "ref": "A", "alt": "G"}, 1),  # Adjacent position
            
            ({"chr": "1", "pos": 12345, "ref": "A", "alt": "G"},
             {"chr": "1", "pos": 12445, "ref": "A", "alt": "G"}, 100),  # 100bp apart
            
            ({"chr": "1", "pos": 12345, "ref": "A", "alt": "G"},
             {"chr": "2", "pos": 12345, "ref": "A", "alt": "G"}, 1000),  # Different chromosome
        ]
        ...
        for var1, var2, expected_distance in variant_pairs:
            ...
            # Log metrics
            metrics.record("hdc_similarity_test", similarity, 
                          metadata={"edit_distance": expected_distance})
        
        # Assertions
        preservation_rate = sum(r["preserved"] for r in results) / len(results)
        assert preservation_rate >= 0.9, f"Similarity preservation rate too low: {preservation_rate}"
        ...
        decreasing_count = sum(similarities[i] >= similarities[i+1] 
    ...
    def test_compression_ratio(self, encoder):
        ...
        for strategy, compressed_size in storage_strategies.items():
            ...
            metrics.record("hdc_compression_ratio", ratio, 
    ...
    def test_discrimination_ability(self, encoder):
        ...
        assert all(s > 0.5 for s in similar_similarities), \
        ...
        assert low_dissimilar >= len(dissimilar_similarities) * 0.8, \
    ...
    def test_clinical_variant_preservation(self, encoder):
        ...
        pathogenic_variants = [
            ...
            {"chr": "7", "pos": 117559590, "ref": "G", "alt": "A", "gene": "CFTR"},  # Cystic fibrosis
        ]
        ...
        # Analyze results
        results = {
            "avg_pathogenic_similarity": np.mean(pathogenic_similarities) if pathogenic_similarities else 0,
            ...
            "separation_score": (np.mean(pathogenic_similarities) + np.mean(benign_similarities)) / 2 - np.mean(cross_similarities)
        }
    ...
    def test_batch_processing_efficiency(self, encoder):
        ...
        for batch_size in batch_sizes:
            ...
            if hasattr(encoder, 'encode_variants_batch'):
                assert np.allclose(individual_hvs, batch_hvs), "Batch encoding differs from individual"
            
            results[batch_size] = {
                "variants_encoded": batch_size,
                "individual_time": metrics.get_summary().get(f"individual_encoding_{batch_size}_time", {}).get("mean", 0),
                "batch_time": metrics.get_summary().get(f"batch_encoding_{batch_size}_time", {}).get("mean", 0)
            }
...
def run_hdc_quality_assessment():
    ...
    print(f"   Preservation rate: {sum(r['preserved'] for r in similarity_results) / len(similarity_results):.2%}")
