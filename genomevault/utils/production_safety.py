...
def validate_not_mock(proof: Any) -> bool:
    """
    ...
    for indicator in mock_indicators:
        if indicator in proof_data or indicator in proof_str:
            error_msg = f"Mock proof detected: contains '{indicator}'"
            ...
            if is_production():
                ...
                logger.warning(f"STAGING WARNING: {error_msg}")
                return False
            else:
    ...
    if isinstance(proof_data, dict):
        ...
        if proof_data_size < 100:  # Real proofs are typically much larger
            error_msg = f"Suspicious proof size: {proof_data_size} bytes (expected >100)"
...
def validate_proof_structure(proof: Any) -> bool:
    ...
    if has_groth16_fields:
        missing_fields = [field for field in expected_groth16_fields 
...
def fail_loud_in_production(error_msg: str, exception: Optional[Exception] = None):
    """
    ...
    full_msg = f"GENOMEVAULT SAFETY VIOLATION: {error_msg}"
    ...
    if is_production():
        ...
        logger.error(f"STAGING ERROR (would fail in prod): {full_msg}")
...
def require_secure_environment(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        for var in insecure_env_vars:
            if os.environ.get(var, '').lower() in ['true', '1', 'yes']:
                error_msg = f"Insecure environment variable {var} is enabled"
...
def get_environment_info() -> Dict[str, Any]:
    ...
    return {
        ...
        'security_disabled': os.environ.get('GENOMEVAULT_DISABLE_SECURITY', 'false').lower() == 'true',
    }
