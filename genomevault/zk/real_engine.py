...
class RealZKEngine:
    ...
    def verify_proof(
        ...
        if isinstance(proof, dict) and proof.get("engine") == "transcript":
            ...
            if transcript_public_str != provided_public_str:
                logger.warning(f"Public inputs mismatch: expected {transcript_public_str}, got {provided_public_str}")
                return False
    ...
    def _public_dict_to_array(
        ...
        if circuit_type == "sum64":
            ...
            return [
                str(public_dict.get("median", 0)),
                str(public_dict.get("count", 0))
            ]
        else:
            # Try to preserve order or use sorted keys
            return [str(v) for v in public_dict.values()]
    
    def _create_transcript_proof(
        self,
        circuit_type: str,
        inputs: Dict[str, Any]
    ) -> RealProof:
