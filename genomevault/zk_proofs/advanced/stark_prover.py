...
class STARKProver:
    ...
    def generate_stark_proof(
        ...
        """
        logger.info(
            f"Generating STARK proof for trace of size "
            f"{computation_trace.shape}"
        )
        ...
        logger.info(
            f"Generated STARK proof: {proof.proof_id}, "
            f"size: {proof.proof_size_kb:.1f}KB"
        )
        
        return proof
    
    def _commit_to_trace(
        self,
        trace: np.ndarray
    ...
    def _generate_constraint_polynomial(
        ...
        for constraint in constraints:
            if constraint["type"] == "boundary":
                ...
                poly = self._transition_constraint_poly(
                    trace,
    ...
    def _reed_solomon_encode(self, data: np.ndarray) -> np.ndarray:
        ...
        evaluation_domain = self._get_evaluation_domain(
            len(data) * blowup_factor
        )
...
if __name__ == "__main__":
    ...
    stark_proof = prover.generate_stark_proof(
        trace,
        public_inputs,
        constraints
    )
