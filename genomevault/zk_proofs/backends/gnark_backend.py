...
class GnarkBackend:
    ...
    def __init__(self, circuit_dir: str = "./circuits/compiled", 
                 gnark_path: Optional[str] = None):
        """
    ...
    def _find_gnark_path(self) -> str:
        ...
        # Try to find in PATH
        result = subprocess.run(["which", "gnark-prover"], 
    ...
    def _verify_binaries(self):
        ...
        # Test execution
        try:
            result = subprocess.run([str(self.prover_bin), "--version"],
    ...
    def _compile_circuit(self, circuit_name: str):
        ...
        cmd = [
            ...
            "--curve", "bn254",  # BN254 curve for efficiency
        ]
    ...
    def generate_proof(self, circuit_name: str, 
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            cmd = [
                str(self.prover_bin),
                "--circuit", circuit_info["r1cs_path"],
            ...
            metrics.record(f"{circuit_name}_generation_time", 
                         generation_time * 1000, "ms")
            
            logger.info(
                f"Generated {circuit_name} proof: "
                f"{proof_size} bytes in {generation_time:.3f}s"
            )
            
            return proof_data
    
    def verify_proof(self, circuit_name: str, 
                    proof: bytes,
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Record metrics
            metrics.record(f"{circuit_name}_verification_time", 
                         verification_time * 1000, "ms")
