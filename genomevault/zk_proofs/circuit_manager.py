...
from .circuits.biological.diabetes import (DiabetesRiskCircuit,
...
from .circuits.biological.multi_omics import (
    ClinicalTrialEligibilityCircuit, GenotypePhenotypeAssociationCircuit,
    MultiOmicsCorrelationCircuit, RareVariantBurdenCircuit)
...
class CircuitManager:
    ...
    def _optimize_circuit_selection(
        ...
        candidates.sort(key=lambda x: x[1].constraint_count / x[1].proof_size_bytes)
        ...
        if "target_proof_size" in target_metrics:
            ...
            if circuit_name == "polygenic_risk_score":
                ...
                optimized_params["max_variants"] = min(
                    max_variants, optimized_params.get("max_variants", 1000)
                )
