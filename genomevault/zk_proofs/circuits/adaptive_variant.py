...
class SmallVariantCircuit(VariantCircuit):
    ...
    def generate_witness(self, inputs: Dict) -> Dict:
        ...
        for i, var in enumerate(variants[: self.buffer_size]):
            if (
                var["chr"] == query["chr"]
                and var["pos"] == query["pos"]
                and var["alt"] == query["alt"]
            ):
                found = True
                index = i
                break

        return {"found": found, "index": index, "num_variants": len(variants)}
...
class AdaptiveVariantPresenceCircuit:
    ...
    def auto_tune(self):
        ...
        if self.performance_stats["small"]["count"] > 100:
            small_avg = (
                self.performance_stats["small"]["total_time"]
        ...
        if self.performance_stats["large"]["count"] > 100:
            large_avg = (
                self.performance_stats["large"]["total_time"]
    ...
    def get_performance_stats(self) -> Dict:
        ...
        for circuit_type in ["small", "large"]:
            ...
            if count > 0:
                ...
                stats[circuit_type] = {"count": 0, "avg_time_ms": 0, "total_time_s": 0}
