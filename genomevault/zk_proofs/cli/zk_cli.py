...
def cmd_verify(args):
    ...
    if args.public_input:
        load_json_file(args.public_input)
    else:
        # Extract from proof
...
def cmd_demo(args):
    ...
    if args.circuit == "variant_presence":
        ...
        private_inputs = {
            ...
            "merkle_proofs": [{"path": ["node1", "node2"], "indices": [0, 1]}] * 5,
...
Examples:
  ...
  zk_info --circuit diabetes_risk
    ...
    verify_parser.add_argument(
        "--verbose", "-v", action="store_true", help="Show detailed verification info"
    )
    ...
    demo_parser.add_argument(
        "--save-inputs", "-s", action="store_true", help="Save example inputs to files"
    )
