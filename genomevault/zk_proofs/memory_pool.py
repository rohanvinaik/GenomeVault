...
class MemoryEfficientProver:
    ...
    def generate_witness_with_pool(
        ...
        workspace = self.memory_manager.allocate_workspace(
            circuit_type,
            ['constraint_generation', 'witness_computation']
        )
    ...
    def _compute_with_buffers(
        ...
        proof = self.base_prover.generate_proof(
            circuit_type,
            public_inputs,
            private_inputs
        )
    ...
    def batch_generate_with_pool(
        ...
        for circuit_type, public_inputs, private_inputs in tasks:
            witness = self.generate_witness_with_pool(
                circuit_type,
                public_inputs,
                private_inputs
            )
