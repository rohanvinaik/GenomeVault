...
class PerformanceMonitor:
    ...
    def _update_stats(self, metric: PerformanceMetric) -> None:
        ...
        if metric.circuit_type not in self.circuit_stats:
            self.circuit_stats[metric.circuit_type] = CircuitStats(
                circuit_type=metric.circuit_type
            )
    ...
    def _check_alerts(self, metric: PerformanceMetric) -> None:
        ...
        # High latency alert
        if metric.operation == 'witness' and metric.duration_ms > self.alert_thresholds['witness_ms']:
            self.alerts.append({
                'type': 'high_latency',
        ...
        if metric.circuit_type in self.circuit_stats:
            ...
            if stats.total_operations > 100 and stats.cache_hit_rate < self.alert_thresholds['cache_hit_rate']:
                self.alerts.append({
    ...
    def get_dashboard_data(self) -> Dict[str, Any]:
        ...
        for circuit, stats in self.circuit_stats.items():
            circuit_data[circuit] = {
                ...
                'recent_throughput': sum(stats.hourly_throughput) / max(1, len(stats.hourly_throughput))
            }
