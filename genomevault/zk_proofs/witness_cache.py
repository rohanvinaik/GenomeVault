...
class WitnessCache:
    ...
    def _generate_cache_key(self, circuit_name: str, inputs: Dict[str, Any]) -> str:
    ...
    def get_or_compute(
        self, circuit_name: str, inputs: Dict[str, Any], compute_fn: Callable
    ...
    def warm_cache(self, common_patterns: List[Tuple[str, Dict]]) -> None:
        ...
        for circuit_name, inputs in common_patterns:
            try:
                self.get_or_compute(
                    circuit_name, inputs, lambda c, i: prover._generate_witness_direct(c, i)
                )
                warmed += 1
