...
class OpenAIIntegration:
    ...
    def __init__(self, config: Optional[OpenAIConfig] = None):
        ...
        logger.info(f"Initialized OpenAI integration with model {self.config.default_model.value}")

    async def analyze_literature(
        self, query: str, focus_areas: List[str], max_results: int = 10
    ) -> Dict[str, Any]:
        """
...
Limit to {max_results} most relevant findings."""
    ...
    async def generate_embeddings(
        ...
        try:
            response = await self.client.post(
                f"{self.config.api_url}/embeddings", json={"input": texts, "model": model}
            )
    ...
    async def _call_gpt(
        self, prompt: str, system: Optional[str] = None, temperature: Optional[float] = None
    ) -> str:
        """
