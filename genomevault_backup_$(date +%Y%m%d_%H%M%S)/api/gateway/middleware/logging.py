...
class LoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def _log_performance_metrics(
        ...
        if process_time > 5.0:  # More than 5 seconds
            logger.warning(f"Slow request detected: {process_time:.2f}s", extra=metrics_data)
        else:
    ...
    async def _log_error(
        ...
        logger.error(
            f"Request error: {type(error).__name__}: {str(error)}", extra=error_data, exc_info=True
        )
