...
class PHIDataProtector:
    ...
    def sanitize_data_for_audit(cls, data: Any, context: str = "") -> Any:
        ...
        if isinstance(data, dict):
            ...
            return [
                cls.sanitize_data_for_audit(item, f"{context}[{i}]") for i, item in enumerate(data)
            ]
...
class AuditLogger:
    ...
    def _sign_event(self, event_data: Dict[str, Any]) -> str:
        ...
        return hmac.new(
            self.signing_key.encode(), canonical_data.encode(), hashlib.sha256
        ).hexdigest()
    ...
    def __init__(self, app, audit_logger: Optional[AuditLogger] = None):
        ...
        if audit_logger is None:
            ...
            audit_logger = AuditLogger(
                log_file_path=log_file, signing_key=signing_key, enable_console_output=True
            )
