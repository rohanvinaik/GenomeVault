...
class AuthenticationMiddleware(BaseHTTPMiddleware):
    ...
    async def _validate_api_key(self, api_key: str, request: Request) -> User:
        ...
        if api_key_obj.rotation_interval_days and api_key_obj.last_rotated_at:
            rotation_due = api_key_obj.last_rotated_at + timedelta(
                days=api_key_obj.rotation_interval_days
            )
        ...
        if not user:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED, detail="User not found for API key"
            )
    ...
    async def _validate_jwt_token(self, token: str, request: Request) -> User:
...
def generate_api_key(
    user_id: str, name: str, scopes: List[str], expires_in_days: Optional[int] = 365, **kwargs
) -> tuple[str, APIKey]:
...
def rotate_api_key(old_key_id: str, user_id: str) -> tuple[str, APIKey]:
    ...
    if old_key_data.rotation_interval_days:
        new_key_obj.expires_at = datetime.now(timezone.utc) + timedelta(
            days=old_key_data.rotation_interval_days
        )
...
class MFAVerifier:
    ...
    @staticmethod
    def generate_totp_uri(secret: str, username: str, issuer: str = "GenomeVault") -> str:
    ...
    async def send_sms_code(phone: str) -> str:
        ...
        redis_client.setex(f"mfa_sms:{phone}", timedelta(minutes=5), code)
    ...
    async def send_email_code(email: str) -> str:
        ...
        redis_client.setex(f"mfa_email:{email}", timedelta(minutes=10), code)
