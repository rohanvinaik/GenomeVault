...
class RateLimitManager:
    ...
    DEFAULT_CONFIGS = {
        ...
        EndpointSensitivity.COMPUTE: RateLimitConfig(
            requests_per_minute=30, requests_per_hour=500, requests_per_day=5000, burst_allowance=10
        ),
        EndpointSensitivity.CLINICAL: RateLimitConfig(
            requests_per_minute=20, requests_per_hour=200, requests_per_day=1000, burst_allowance=5
        ),
        EndpointSensitivity.ADMIN: RateLimitConfig(
            requests_per_minute=10, requests_per_hour=100, requests_per_day=500, burst_allowance=2
        ),
    }
    ...
    def _get_endpoint_sensitivity(self, path: str, method: str) -> EndpointSensitivity:
        ...
        if any(
            pattern in path_lower for pattern in ["/hdc/", "/zk/", "/pir/", "/encode/", "/prove/"]
        ):
    ...
    def _token_bucket_check(
        ...
        if self.redis_client:
            ...
            bucket_data = self._fallback_storage.get(
                bucket_key, {"last_refill": current_time, "tokens": config.requests_per_minute}
            )
        ...
        # Update bucket
        bucket_data = {"last_refill": current_time, "tokens": tokens}
    ...
    def _sliding_window_check(
        ...
        for window_seconds, limit in config.get_window_limits().items():
            ...
            if self.redis_client:
                ...
                headers[f"X-RateLimit-Remaining-{window_seconds}s"] = max(
                    0, limit - len(window_data)
                )
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def _log_rate_limit_exceeded(self, request: Request):
        ...
        print(
            f"Rate limit exceeded: client_hash={client_id_hash}, "
...
def create_rate_limit_middleware(
    redis_url: Optional[str] = None, enable_rate_limiting: bool = True
) -> RateLimitMiddleware:
    ...
    return lambda app: RateLimitMiddleware(
        app, redis_client=redis_client, enable_rate_limiting=enable_rate_limiting
    )
