        ...
        csp = self._build_csp(
            {
                ...
                CSPDirective.SCRIPT_SRC: [
                    "'self'",
                    "'unsafe-inline'",
                ],  # Some inline scripts for staging
...
def create_security_headers_middleware(
    profile: Optional[SecurityProfile] = None, custom_headers: Optional[Dict[str, str]] = None
) -> callable:
    ...
    def middleware_factory(app):
        return SecurityHeadersMiddleware(app=app, profile=profile, custom_headers=custom_headers)
...
def validate_security_headers(headers: Dict[str, str], profile: SecurityProfile) -> List[str]:
    ...
    for header, expected_value in required_headers.items():
        ...
        if not actual_value:
            ...
            issues.append(
                f"Incorrect header value for {header}: expected '{expected_value}', got '{actual_value}'"
            )
