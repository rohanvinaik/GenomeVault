...
data_exports = Counter(
    "genomevault_data_exports_total", "Total data exports for portability", ["format"]
)
...
class DataPortability:
    ...
    def export_user_data(
        self, user_id: str, format: DataFormat, include_derived: bool = False
    ) -> Path:
    ...
    def _export_json(self, data: Dict[str, Any], user_id: str) -> Path:
        ...
        export_path = (
            export_dir / f"gdpr_export_{user_id}_{datetime.utcnow().strftime('%Y%m%d')}.json"
        )
    ...
    def _export_fhir(self, data: Dict[str, Any], user_id: str) -> Path:
        ...
        patient = {
            ...
            "id": user_id,
            "identifier": [{"system": "https://genomevault.io", "value": user_id}],
        }
        ...
        for variant in data.get("genomic_data", {}).get("variants", []):
            observation = {
                ...
                "component": [
                    {"code": {"text": "Chromosome"}, "valueString": variant["chrom"]},
                    {"code": {"text": "Position"}, "valueInteger": variant["pos"]},
                ],
            }
...
class DataResidencyController:
    ...
    def check_residency_compliance(
        ...
        if user_country in self.EEA_COUNTRIES:
            result = self._check_eea_compliance(
                user_country, storage_region, data_type, transfer_mechanism
            )
        ...
        # Check adequacy decisions
            ...
            result = self._check_adequate_country_compliance(
    ...
    def _check_eea_compliance(
        ...
        if transfer_mechanism in self.TRANSFER_MECHANISMS:
            ...
            return {
                "compliant": False,
                "violations": [f"EEA data transferred to {storage_region} without valid mechanism"],
            }

    def _check_adequate_country_compliance(
        self, user_country: str, storage_region: str, data_type: str
    ) -> Dict[str, Any]:
        ...
        return {
            "compliant": True,
            "warnings": [
                f"Data from {user_country} stored in {storage_region} - monitor for changes in adequacy status"
            ],
        }
    ...
    def _log_transfer(
        ...
        if not compliance_result["compliant"]:
            logger.warning(
                f"Non-compliant cross-border transfer: {user_country} -> {storage_region}"
            )

    def get_compliant_storage_region(
        self,
        user_country: str,
        data_type: str = "personal_data",
        ...
        if user_country in self.EEA_COUNTRIES:
            eu_regions = ["eu-west-1", "eu-west-2", "eu-west-3", "eu-central-1", "eu-north-1"]
    ...
    def _select_optimal_region(self, regions: List[str], performance_preference: str) -> str:
    ...
    def conduct_transfer_impact_assessment(
        self, source_country: str, target_country: str, data_categories: List[str]
    ) -> Dict[str, Any]:
    ...
    def _calculate_transfer_risk(
        self, legal: Dict[str, Any], technical: Dict[str, Any], organizational: Dict[str, Any]
    ) -> str:
    ...
    def _get_tia_recommendations(self, risk_level: str) -> List[str]:
        ...
        if risk_level == "HIGH":
            ...
            recommendations.extend(["Standard safeguards sufficient", "Annual review recommended"])
    ...
    def generate_residency_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "compliance_rate": (
                (len(recent_logs) - len(violations)) / len(recent_logs) * 100
                if recent_logs
                else 100
            ),
...
class GDPRHandler:
    ...
    def create_request(
        self, user_id: str, request_type: RequestType, metadata: Optional[Dict[str, Any]] = None
    ) -> DataSubjectRequest:
        ...
        request.add_audit_entry(
            "request_created", {"type": request_type.value, "metadata": metadata}
        )

        self.pending_requests[request_id] = request

        gdpr_requests.labels(request_type=request_type.value, status="created").inc()
    ...
    def verify_request(self, request_id: str, token: str) -> bool:
        ...
        gdpr_requests.labels(request_type=request.request_type.value, status="verified").inc()
    ...
    def _process_request(self, request: DataSubjectRequest) -> None:
        ...
        with gdpr_request_duration.labels(request_type=request.request_type.value).time():
        ...
        gdpr_requests.labels(request_type=request.request_type.value, status="completed").inc()
    ...
    def _process_erasure_request(self, request: DataSubjectRequest) -> None:
        ...
        if self._has_legal_obligation(user_id):
            ...
            request.add_audit_entry(
                "erasure_rejected",
                {"reason": "legal_obligation", "details": "HIPAA 7-year retention requirement"},
            )
            return
        ...
        if success:
            request.add_audit_entry(
                "data_erased",
                {"method": "cryptographic_erasure", "timestamp": datetime.utcnow().isoformat()},
            )
    ...
    def _process_portability_request(self, request: DataSubjectRequest) -> None:
        ...
        export_path = self.data_portability.export_user_data(
            request.user_id, format, include_derived=request.metadata.get("include_derived", False)
        )

        request.metadata["export_path"] = str(export_path)
        request.add_audit_entry("data_exported", {"format": format.value, "path": str(export_path)})
    ...
    def _process_restriction_request(self, request: DataSubjectRequest) -> None:
        ...
        request.add_audit_entry("processing_restricted", {"type": restriction_type})
    ...
    def _process_objection_request(self, request: DataSubjectRequest) -> None:
        ...
        request.add_audit_entry("objection_recorded", {"scope": objection_scope})

    def _send_verification_email(self, request: DataSubjectRequest) -> None:
        """Send verification email for request."""
        verification_link = (
            f"{self.verification_url}?id={request.request_id}&token={request.verification_token}"
        )
