...
audit_events = Counter(
    "genomevault_audit_events_total", "Total audit events logged", ["event_type", "outcome"]
)
...
audit_integrity_checks = Counter(
    "genomevault_audit_integrity_checks_total", "Audit log integrity checks", ["result"]
)

audit_log_size = Gauge("genomevault_audit_log_size_bytes", "Current audit log size in bytes")
...
class BlockchainAnchor:
    ...
    def add_event(self, event: AuditEvent) -> None:
        ...
        if (
            len(self.pending_events) >= self.batch_size
            or time.time() - self.last_anchor_time > self.batch_interval
        ):
            self._anchor_batch()
...
class AuditDatabase:
    ...
    def query_events(
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for row in cursor:
                ...
                event_data["additional_details"] = json.loads(
                    event_data["additional_details"] or "{}"
                )
    ...
    def check_integrity(self) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            audit_integrity_checks.labels(
                result="success" if integrity_issues == 0 else "failure"
            ).inc()
...
class HIPAAAuditor:
    ...
    def __init__(
        self, db_path: str = "/var/genomevault/audit/audit.db", enable_blockchain: bool = True
    ):
    ...
    def log_event(
        ...
        **kwargs,
        ...
        # Update metrics
        audit_events.labels(event_type=event_type.value, outcome=outcome.value).inc()
    ...
    def _handle_failed_login(self, user_id: str, ip_address: str) -> None:
        ...
        self.failed_login_tracker[ip_address] = [
            attempt for attempt in self.failed_login_tracker[ip_address] if attempt > cutoff
        ]
