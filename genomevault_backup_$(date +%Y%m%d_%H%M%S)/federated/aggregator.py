...
class SecureAggregator:
    ...
    def __init__(self, num_clients: int, vector_size: int, seed: Optional[int] = None):
        ...
        logger.info(
            f"Initialized SecureAggregator for {num_clients} clients, " f"vector size {vector_size}"
        )
    ...
    def mask_update(self, update: np.ndarray, client_id: int) -> np.ndarray:
        ...
        if len(update) != self.vector_size:
            raise ValueError(f"Update size {len(update)} doesn't match expected {self.vector_size}")
    ...
    def aggregate_masked(self, masked_updates: List[np.ndarray]) -> np.ndarray:
        ...
        if len(masked_updates) != self.num_clients:
            raise ValueError(f"Expected {self.num_clients} updates, got {len(masked_updates)}")
        ...
        if self.use_differential_privacy and DP_AVAILABLE:
            ...
            if not self.privacy_accountant:
                self.privacy_accountant = PrivacyAccountant(
                    total_epsilon=epsilon * 100, total_delta=delta * 100
                )
            ...
            logger.info(f"Differential privacy enabled for FedAvg: ε={epsilon}, δ={delta}")
            ...
            try:
                ...
                if self.privacy_accountant:
                    params = self.privacy_accountant.allocate_budget(
                        "federated", "aggregate", self.base_epsilon
                    )
                    params.sensitivity = sensitivity

                    # Create mechanism with allocated budget
                    self.dp_mechanism = GaussianMechanism(
                        params.epsilon, params.delta, params.sensitivity
                    )
                else:
                    # Use base parameters
                    self.dp_mechanism = GaussianMechanism(
                        self.base_epsilon, self.base_delta, sensitivity
                    )
