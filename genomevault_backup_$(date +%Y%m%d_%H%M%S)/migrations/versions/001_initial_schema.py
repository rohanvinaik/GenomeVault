...
def upgrade() -> None:
    ...
    # Create custom types
    op.execute(
        "CREATE TYPE user_role AS ENUM ('patient', 'clinician', 'researcher', 'admin', 'service');"
    )
    op.execute(
        "CREATE TYPE audit_event_type AS ENUM ('login', 'logout', 'data_access', 'data_modification', 'phi_access', 'export', 'api_call', 'error');"
    )
    op.execute(
        "CREATE TYPE data_classification AS ENUM ('public', 'internal', 'confidential', 'phi', 'restricted');"
    )
    op.execute(
    ...
    op.create_table(
        ...
        sa.Column(
            "retention_tier",
            sa.Enum("hot", "warm", "cold", "archive", name="retention_tier"),
            default="hot",
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "lsh_buckets", postgresql.ARRAY(sa.Integer), nullable=True
        ...
        sa.ForeignKeyConstraint(
            ["genomic_data_id"], ["genomic_data_metadata.id"], ondelete="CASCADE"
        ),
        sa.ForeignKeyConstraint(
            ["previous_version_id"], ["hypervector_storage.id"], ondelete="SET NULL"
        ),
    ...
    op.create_table(
        ...
        # Noise parameters
        sa.Column(
            "noise_mechanism", sa.String(50), nullable=False
    ...
    op.create_table(
        ...
        sa.Column(
            "tier_transitions", postgresql.JSONB(), nullable=True
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.Index("idx_backup_history_time", "started_at"),
