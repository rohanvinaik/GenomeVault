...
class PHILeakageDetector:
    ...
    def _load_patterns(self) -> Dict[PHICategory, List[re.Pattern]]:
        ...
        patterns = {
            PHICategory.SSN: [re.compile(r"\b\d{3}-\d{2}-\d{4}\b"), re.compile(r"\b\d{9}\b")],
        ...
        if self.sensitivity == "low":
            ...
            patterns[PHICategory.ADDRESS] = [
                re.compile(
                    r"\b\d+\s+[A-Za-z\s]+\s+(Street|St|Avenue|Ave|Road|Rd|Lane|Ln|Drive|Dr)\b", re.I
                )
            ]
    ...
    def _calculate_confidence(
        self, category: PHICategory, value: str, context: str, position: tuple
    ) -> float:
        """
        ...
        if category == PHICategory.SSN:
            ...
            if any(
                keyword in context.lower() for keyword in ["variant", "snp", "mutation", "sequence"]
            ):
                confidence = 0.9
            else:
                confidence = 0.5
...
class StructuredLogger:
    ...
    def request_context_manager(
        ...
        try:
            self.info("Request started", extra={"event": "request_start"})
            yield request_id
        except Exception as e:
            self.error(
                f"Request failed: {str(e)}", extra={"event": "request_error", "error": str(e)}
            )
            raise
        finally:
            duration = time.time() - self.request_context["start_time"]
            self.info(
                "Request completed", extra={"event": "request_end", "duration_ms": duration * 1000}
            )
    ...
    def _prepare_log_data(self, msg: str, extra: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        Returns:
            Prepared log data
        """
        data = {"message": msg, "timestamp": datetime.utcnow().isoformat(), **self.request_context}
    ...
    def audit(
        self, action: str, resource: str, outcome: str, details: Optional[Dict] = None
    ) -> None:
        """
        Log audit event.
    ...
    def metric(self, name: str, value: float, unit: str, tags: Optional[Dict] = None) -> None:
        """
        Log metric event.
    ...
    def add_fields(self, log_record: Dict, record: logging.LogRecord, message_dict: Dict):
        ...
        if self.phi_detector and self.enable_phi_detection:
            ...
            if detections:
                ...
                log_record["phi_categories_detected"] = list(
                    set(d.category.value for d in detections)
                )
...
def get_logger(
    name: str, level: Optional[str] = None, enable_phi_detection: bool = True
) -> StructuredLogger:
    """
    ...
    if name not in _loggers:
        ...
        _loggers[name] = StructuredLogger(
            name=name, level=level, enable_phi_detection=enable_phi_detection, output_file=log_file
        )
