...
class ConstantTimeQueryProcessor:
    ...
    def process_query(
        self, query: PIRQuery, database: np.ndarray, server_id: int, client_id: str
    ) -> PIRResponse:
        """
    ...
    def _constant_time_computation(
        self, database: np.ndarray, query_vector: np.ndarray
    ) -> np.ndarray:
        """
    ...
    def batch_process_queries(
        self, queries: List[Tuple[PIRQuery, str]], database: np.ndarray, server_id: int
    ) -> List[PIRResponse]:
        """
    ...
    def combine_server_responses(self, responses: Dict[int, PIRResponse]) -> Optional[bytes]:
        """
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self.processing_lock:
            ...
            stats["cache_hit_rate"] = (
                stats["cache_hits"] / stats["total_queries"] if stats["total_queries"] > 0 else 0
            )
