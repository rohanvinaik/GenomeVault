    ...
    def _create_fourier_basis(self) -> torch.Tensor:
        ...
        for i, freq in enumerate(frequencies):
            ...
            if 2 * i < self.dimension:
                basis[2 * i] = torch.cos(2 * np.pi * freq * torch.arange(self.dimension) / self.dimension)
            # Sine component  
            if 2 * i + 1 < self.dimension:
                basis[2 * i + 1] = torch.sin(2 * np.pi * freq * torch.arange(self.dimension) / self.dimension)
...
class HierarchicalEncoder:
    ...
    def _create_oncology_projection(self, dim: int) -> torch.nn.Module:
        ...
        class OncologyProjection(torch.nn.Module):
            def __init__(self, input_dim: int, output_dim: int):
                ...
                self.cancer_gene_weights = torch.nn.Parameter(
    ...
    def _create_population_projection(self, dim: int) -> torch.nn.Module:
        ...
        class PopulationProjection(torch.nn.Module):
            ...
            def forward(self, x: torch.Tensor) -> torch.Tensor:
                ...
                combined = torch.cat([x.unsqueeze(1).expand(-1, 26, -1), 
                                    pop_embeds.unsqueeze(2)], dim=2)
    ...
    def encode_hierarchical(self, features: Union[torch.Tensor, np.ndarray, Dict],
                          omics_type: OmicsType,
                          domain: ProjectionDomain = ProjectionDomain.GENERAL) -> HierarchicalHypervector:
        """
    ...
    def bind_hypervectors(self, hv1: HierarchicalHypervector,
                         hv2: HierarchicalHypervector,
                         operation: str = "circular") -> HierarchicalHypervector:
        """
        ...
        return HierarchicalHypervector(
            ...
            metadata={
                ...
                'source_omics': [hv1.metadata.get('omics_type'), 
    ...
    def similarity_multiresolution(self, hv1: HierarchicalHypervector,
                                  hv2: HierarchicalHypervector,
    ...
    def compress_to_tier(self, hv: HierarchicalHypervector,
                        tier: str = "clinical") -> torch.Tensor:
        """
...
def encode_genomic_hierarchical(genomic_data: Dict,
                              domain: ProjectionDomain = ProjectionDomain.GENERAL) -> HierarchicalHypervector:
...
if __name__ == "__main__":
    ...
    for domain in [ProjectionDomain.GENERAL, ProjectionDomain.ONCOLOGY, 
    ...
    hv2 = encoder.encode_hierarchical({'expression_matrix': {'BRCA1': 2.5, 'TP53': 1.8}}, 
