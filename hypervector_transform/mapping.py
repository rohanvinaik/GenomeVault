...
class SimilarityPreservingMapper:
    ...
    def _optimize_mapping(self, data: torch.Tensor,
                         target_similarities: torch.Tensor):
        """Optimize mapping matrix to preserve similarities"""
        optimizer = torch.optim.Adam([self.mapping_matrix], 
        ...
        for iteration in range(self.config.num_iterations):
            ...
            if self.config.preserve_angles:
                ...
                angle_loss = self._angle_preservation_loss(
                    data, transformed
                )
                loss += angle_loss
    ...
    def _angle_preservation_loss(self, original: torch.Tensor,
                                transformed: torch.Tensor) -> torch.Tensor:
    ...
    def _neighborhood_preservation_loss(self, original_sim: torch.Tensor,
                                      transformed_sim: torch.Tensor) -> torch.Tensor:
...
class BiologicalSimilarityMapper(SimilarityPreservingMapper):
    ...
    def compute_biological_similarity(self, data1: torch.Tensor,
                                    data2: torch.Tensor,
                                    similarity_type: str = "default") -> float:
        """
...
class ManifoldPreservingMapper:
    ...
    def fit_transform(self, data: torch.Tensor) -> torch.Tensor:
        ...
        _, neighbors = torch.topk(distances, k=self.n_neighbors + 1, 
                                 largest=False, dim=1)
    ...
    def _optimize_embedding(self, data: torch.Tensor,
        ...
        for iteration in range(n_iterations):
            ...
            if iteration % 100 == 0:
                logger.debug(f"Manifold optimization iteration {iteration}, Loss: {loss.item():.4f}")


# Convenience functions
def create_biological_mapper(input_dim: int, output_dim: int,
                           omics_type: OmicsType) -> BiologicalSimilarityMapper:
