...
class PhenotypeProcessor:
    ...
    def process(self, 
                ...
                sample_id: str,
                data_format: str = "fhir") -> PhenotypeProfile:
        """
        ...
        logger.info(f"Phenotype processing complete. "
    ...
    def _process_fhir_data(self, fhir_bundle: Dict[str, Any], sample_id: str) -> PhenotypeProfile:
        ...
        return PhenotypeProfile(
            ...
            metadata={'source': 'fhir', 'version': fhir_bundle.get('meta', {}).get('versionId')}
        )
    ...
    def _extract_fhir_observation(self, observation: Dict[str, Any]) -> Optional[ClinicalMeasurement]:
    ...
    def _extract_fhir_condition(self, condition: Dict[str, Any]) -> Optional[Diagnosis]:
        ...
        diagnosis = Diagnosis(
            ...
            status='active' if condition.get('clinicalStatus', {}).get('coding', [{}])[0].get('code') == 'active' else 'resolved'
        )
    ...
    def _process_custom_data(self, custom_data: Dict[str, Any], sample_id: str) -> PhenotypeProfile:
        ...
        return PhenotypeProfile(
            ...
            measurements=[
                ClinicalMeasurement(**m) 
                for m in custom_data.get('measurements', [])
            ],
            diagnoses=[
                Diagnosis(**d) 
                for d in custom_data.get('diagnoses', [])
            ],
            medications=[
                Medication(**m) 
                for m in custom_data.get('medications', [])
            ],
            family_history=[
                FamilyHistory(**f) 
                for f in custom_data.get('family_history', [])
            ],
    ...
    def _calculate_derived_values(self, profile: PhenotypeProfile) -> PhenotypeProfile:
        ...
        for measurement in profile.measurements:
            if measurement.measurement_type.lower() == 'height' and measurement.unit in ['cm', 'centimeter']:
                height = measurement.value
            elif measurement.measurement_type.lower() == 'weight' and measurement.unit in ['kg', 'kilogram']:
    ...
    def merge_profiles(self,
                      profiles: List[PhenotypeProfile],
                      merge_strategy: str = "most_recent") -> PhenotypeProfile:
        """
        ...
        for profile in profiles[1:]:
            if merge_strategy == "most_recent":
                ...
                for measurement in profile.measurements:
                    ...
                    if not existing or (measurement.date and existing.date and measurement.date > existing.date):
                ...
                merged.diagnoses.extend([d for d in profile.diagnoses if d.diagnosis_id not in diagnosis_ids])
                
                medication_ids = {m.medication_id for m in merged.medications}
                merged.medications.extend([m for m in profile.medications if m.medication_id not in medication_ids])
