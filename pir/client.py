...
class PIRClient:
    ...
    def __init__(self, servers: List[PIRServer], database_size: int):
        ...
        logger.info(f"PIRClient initialized with {len(servers)} servers", 
    ...
    def _validate_servers(self):
        ...
        if len(self.servers) < self.min_servers:
            raise ValueError(
                f"Insufficient servers: {len(self.servers)} < {self.min_servers}"
            )
        ...
        if len(ts_servers) >= self.min_servers:
            ...
            failure_prob = self.calculate_privacy_failure_probability(
                len(ts_servers), 
                config.pir.server_honesty_hipaa
            )
        else:
            # Use generic probability
            failure_prob = self.calculate_privacy_failure_probability(
                len(self.servers),
                config.pir.server_honesty_generic
            )
    ...
    def calculate_min_servers_needed(self, target_failure: float, 
                                   honesty_prob: float) -> int:
        """
    ...
    def create_query(self, target_index: int) -> PIRQuery:
        ...
        query_id = hashlib.sha256(
            f"{target_index}:{time.time()}".encode()
        ).hexdigest()[:16]
        ...
        logger.info(f"PIR query created for index {target_index}", 
                   extra={'privacy_safe': True})
    ...
    async def execute_query(self, query: PIRQuery) -> Any:
        ...
        logger.info(f"PIR query {query.query_id} completed", 
                   extra={'privacy_safe': True})
        
        return result
    
    async def _query_server(self, server: PIRServer, 
                           query: PIRQuery) -> Optional[PIRResponse]:
        """
    ...
    def decode_response(self, response_data: np.ndarray, 
                       data_type: str = 'genomic') -> Any:
        """
