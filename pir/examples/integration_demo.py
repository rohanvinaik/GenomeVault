...
async def demonstrate_network_coordination():
    ...
    print(f"  TS servers: {net_stats['ts_servers']['total']} (healthy: {net_stats['ts_servers']['healthy']})")
    print(f"  LN servers: {net_stats['ln_servers']['total']} (healthy: {net_stats['ln_servers']['healthy']})")
    
    # Get optimal configuration
    config = coordinator.get_server_configuration(
        target_failure_prob=1e-4,
        max_latency_ms=300
    )
    
    print(f"\nAvailable Configurations:")
    for conf in config['configurations']:
        print(f"  {conf['name']}: {conf['total_servers']} servers, "
...
async def main():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for server_id, server in network_info['server_instances'].items():
            ...
            print(f"{server_id}: {stats['server_type']}, "
