...
class PIRServer:
    ...
    def __init__(self, server_id: str, data_directory: Path,
        ...
        self.process_pool = ProcessPoolExecutor(
            max_workers=config.pir.server_workers
        )
        ...
        logger.info(f"PIR server {server_id} initialized", 
                   extra={
    ...
    async def process_query(self, query_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"PIR query {query_id} processed in {computation_time:.1f}ms",
    ...
    async def _compute_dot_product(self, query_vector: np.ndarray,
                                 shard_id: str, 
                                 database_size: int) -> np.ndarray:
        """
        ...
        for chunk_start in range(0, database_size, chunk_size):
            ...
            # Read database chunk
            chunk_data = self._read_database_chunk(
                mmap_data, chunk_indices, item_size
            )
    ...
    def _read_database_chunk(self, mmap_data: mmap.mmap,
                           indices: range,
                           item_size: int) -> List[np.ndarray]:
        """
    ...
    def update_shard(self, shard_id: str, new_data_path: Path,
                    new_checksum: str) -> bool:
        """
...
class TrustedSignatoryServer(PIRServer):
    ...
    def __init__(self, server_id: str, data_directory: Path,
                 npi: str, baa_hash: str, risk_analysis_hash: str,
                 hsm_serial: str):
        """
        ...
        logger.info(f"Trusted Signatory server {server_id} initialized",
