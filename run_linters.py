...
class LinterRunner:
    ...
    def find_python_files(self) -> List[Path]:
        ...
        for directory in self.directories:
            if directory.exists():
                files.extend([f for f in directory.rglob("*.py") 
                ...
                if result.stdout:
                    try:
                        ...
                        filtered = [i for i in issues 
                                   if i['symbol'] not in ['missing-module-docstring', 
                    ...
                    for filepath, issues in sorted(self.results[linter].items()):
                        ...
                        if linter == 'pylint':
                            for issue in issues[:10]:  # Limit to 10 issues per file
                                f.write(f"- **{issue.get('symbol', 'unknown')}**: {issue.get('message', 'No message')}\n")
                                f.write(f"  Line: {issue.get('line', '?')}, Column: {issue.get('column', '?')}\n")
                        else:
                            for issue in issues[:10]:  # Limit to 10 issues per file
                                f.write(f"- {issue}\n")
