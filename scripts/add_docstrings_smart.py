...
def generate_docstring_for_class(node: ast.ClassDef, context: str = "") -> str:
    ...
    if context == "genomevault":
        if 'Hypervector' in name:
            return f'"""Hypervector-based {name.replace("Hypervector", "").lower()} implementation."""'
        elif 'ZK' in name or 'Proof' in name:
            return f'"""Zero-knowledge proof {name.replace("ZK", "").replace("Proof", "").lower()} component."""'
        elif 'PIR' in name:
            return f'"""Private information retrieval {name.replace("PIR", "").lower()} component."""'
    ...
    try:
        ...
        if not ast.get_docstring(tree):
            ...
            for i, line in enumerate(lines):
                if line.strip() and not line.startswith('#') and not line.startswith('from __future__'):
                    insert_line = i
                    break
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                ...
                if not node.name.startswith('_') or node.name in ['__init__', '__str__', '__repr__', '__enter__', '__exit__']:
                    if not ast.get_docstring(node):
