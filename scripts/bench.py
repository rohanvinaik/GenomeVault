...
class GenomeVaultBenchmark:
    ...
    def benchmark_recursive_snark(self) -> BenchmarkResult:
        ...
        for n in num_proofs:
            ...
            for strategy in strategies:
                ...
                result.add_metric(f"{prefix}_verification_time", verify_time * 1000, "ms")
                result.add_metric(f"{prefix}_proof_size", len(recursive_proof.aggregation_proof), "bytes")
                result.add_metric(f"{prefix}_valid", valid)
                
                print(f"  {strategy}: compose={compose_time*1000:.2f}ms, verify={verify_time*1000:.2f}ms")
    ...
    def benchmark_it_pir(self) -> BenchmarkResult:
        ...
        for num_servers, threshold, db_size in configs:
            ...
            result.add_metric(f"{prefix}_query_size_per_server", query.server_queries[0].nbytes, "bytes")
    ...
    def generate_summary(self) -> Dict[str, Any]:
        ...
        for result in self.results:
            ...
            if "claimed_proof_size" in metrics:
                ...
                measured_sizes = [v["value"] for k, v in metrics.items() 
            ...
            if "claimed_verification_time" in metrics:
                ...
                measured_times = [v["value"] for k, v in metrics.items() 
