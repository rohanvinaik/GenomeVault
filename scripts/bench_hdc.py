    ...
    def benchmark_memory_usage(self) -> Dict[str, Any]:
     ...
     """Benchmark memory usage across tiers"""
        ...
        for tier in CompressionTier:
            ...
            print(
                f"  {tier.value}: {memory_kb:.1f} KB (ratio: {compression_ratio:.2f}x, sparsity: {sparsity:.2%})"
            )
     ...
     """Benchmark different projection types"""
        ...
        for proj_type in ProjectionType:
            try:
                ...
                print(
                    f"  {proj_type.value}: {throughput:.1f} ops/s (cache: {cache_stats['memory_mb']:.1f} MB)"
                )
