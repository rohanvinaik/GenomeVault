...
SCANNABLE_EXTENSIONS = {
    ...
    '.md', '.txt', '.rst', '.yaml', '.yml', '.json', '.xml',
    '.sql', '.sh', '.bash', '.dockerfile', '.env'
}
...
SKIP_DIRECTORIES = {
    ...
    '.ruff_cache', 'htmlcov', '.coverage', '.benchmarks'
}
...
class ScanResults:
    ...
    def add_detection(
        ...
        """Add PHI detection."""
        self.detections.append(PHIDetection(
            file_path=file_path,
...
class PHIScanner:
    ...
    def __init__(self, strict_mode: bool = False, check_comments: bool = True, check_docstrings: bool = True):
    ...
    def _scan_line(self, file_path: Path, line_num: int, line: str) -> None:
        ...
        if not self.strict_mode:
            ...
            if any(keyword in line_lower for keyword in ['example', 'test', 'sample', 'dummy', 'fake']):
                return
        ...
        for pattern_type, patterns in GENOMIC_PATTERNS.items():
            for pattern in patterns:
                ...
                for match in matches:
                    ...
                    severity, confidence = self._assess_genomic_risk(pattern_type, match.group(), line, file_path)
    ...
    def _assess_genomic_risk(self, pattern_type: str, match: str, context: str, file_path: Path) -> Tuple[str, float]:
        ...
        if pattern_type in ["genomic_coordinates", "chromosome_notation"]:
            ...
            if any(keyword in context_lower for keyword in ['patient', 'subject', 'individual', 'sample']):
                return "high", 0.9
...
def generate_report(results: ScanResults, format: str = "text") -> str:
    ...
    if results.detections:
        ...
        for detection in sorted_detections:
            ...
            report.append(f"{severity_icon} {detection.severity.upper()} - {detection.pattern_type}")
...
def main():
    ...
    parser.add_argument("--no-comments", dest="check_comments", action="store_false", help="Skip comments")
    parser.add_argument("--no-docstrings", dest="check_docstrings", action="store_false", help="Skip docstrings")
    parser.add_argument("--output-format", choices=["text", "json"], default="text", help="Output format")
    parser.add_argument("--output-file", help="Output file (default: stdout)")
    parser.add_argument("--threshold", choices=["critical", "high", "medium", "low"], default="medium", help="Minimum severity to report")
    ...
    filtered_detections = [
        ...
        if threshold_levels.get(d.severity, 0) >= min_level
    ]
