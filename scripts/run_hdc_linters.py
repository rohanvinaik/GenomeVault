...
def main():
    ...
    flake8_result = run_command(["flake8", str(hdc_module), "--max-line-length=100", "--extend-ignore=E203,W503"])
    results["flake8"] = flake8_result
    
    # 4. Type checking with mypy (if available)
    print("\n4. Running mypy type checker...")
    try:
        mypy_result = run_command(["mypy", str(hdc_module), "--ignore-missing-imports", "--no-strict-optional"])
