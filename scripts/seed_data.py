...
DATABASE_URL = os.getenv(
    "DATABASE_URL", "postgresql://genomevault:genomevault@localhost/genomevault"
)
...
class SeedDataGenerator:
    ...
    async def create_organizations(self, count=5):
        ...
        for i, (name, domain, npi, baa) in enumerate(org_types[:count]):
            ...
            baa_document_hash = (
                hashlib.sha256(f"BAA-{org_id}".encode()).hexdigest() if baa else None
            )
            ...
            self.organizations.append(
                {"id": org_id, "name": name, "domain": domain, "baa_signed": baa}
            )
    ...
    async def create_users(self, count=20):
        ...
        for i in range(count):
            ...
            org_id = (
                random.choice(self.organizations)["id"]
    ...
    async def create_api_keys(self, count=10):
        ...
        for user in api_key_users:
            ...
            """,
                ...
                key_id,
                key_hash,
                f"API Key for {user['username']}",
                f"Development API key for testing",
                user["id"],
                (
                    ["read:genomic", "pir:query", "zk:prove"]
    ...
    async def create_audit_logs(self, count=100):
        ...
        for i in range(count):
            ...
            """,
                ...
                (
                    random.choice(self.genomic_data)["id"]
                    if self.genomic_data and "data" in event_type
                    else None
                ),
    ...
    async def create_data_retention_policies(self):
        ...
        policies = [
            ...
            },
            {
                ...
                "data_type": "genomic",
                "retention_days": 3650,
                "tier_transitions": {"hot_to_warm": 30, "warm_to_cold": 90, "cold_to_archive": 365},
