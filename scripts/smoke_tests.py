...
class SmokeTestRunner:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "result" in result and "privacy_guarantee" in result:
                    privacy_breach_prob = result.get("privacy_guarantee", {}).get("breach_probability", 1.0)
                    ...
                    self.results.append(TestResult(
                        ...
                        False,
                        duration_ms,
                        error="Invalid PIR response format"
                    ))
            else:
                self.results.append(TestResult(
                    "pir_query",
    ...
    async def _run_load_test(self) -> None:
        ...
        async def make_request():
            try:
                response = await self.client.get(
                    urljoin(self.config.base_url, "/api/v1/status")
                )
        ...
        self.results.append(TestResult(
            ...
            error=f"Success rate {success_rate*100:.1f}% below threshold" if success_rate < 0.9 else None
        ))
            ...
            if encode_response.status_code == 200:
                ...
                if decode_response.status_code == 200:
                    ...
                    self.results.append(TestResult(
                        ...
                        integrity_ok,
                        duration_ms,
                        {"original_count": len(test_variants), "decoded_count": len(decoded_data.get("variants", []))},
    ...
    async def _test_pir_privacy(self) -> None:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                privacy_ok = (
                    servers_available >= 2 and
                    breach_probability < self.config.privacy_threshold
                )
                ...
                self.results.append(TestResult(
                    ...
                    error=f"Privacy guarantee breach probability {breach_probability} too high" if not privacy_ok else None
                ))
            else:
                self.results.append(TestResult(
                    "pir_privacy",
    ...
    async def _test_differential_privacy(self) -> None:
        ...
        await self._test_endpoint(
            ...
            check_response=lambda r: (
                "result" in r and
                "privacy_cost" in r and
                r["privacy_cost"]["epsilon"] <= test_data["epsilon"]
            )
        )
    ...
    async def _test_data_anonymization(self) -> None:
        ...
        await self._test_endpoint(
            ...
            expected_status=200,
            check_response=lambda r: (
                "anonymized_data" in r and
                all("patient_id" not in item for item in r["anonymized_data"])
            )
        )
        ...
        if self.config.validate_ssl:
            ...
            try:
                ...
                self.results.append(TestResult(
                    ...
                    error=f"HTTP not redirected: {response.status_code}" if not https_enforced else None
                ))
    ...
    async def _test_authentication(self) -> None:
        ...
        try:
            ...
            self.results.append(TestResult(
                ...
                error=f"No authentication required: {response.status_code}" if not auth_required else None
            ))
    ...
    async def _test_security_headers(self) -> None:
        ...
        try:
            ...
            self.results.append(TestResult(
                ...
                duration_ms,
                {
                    "present_headers": [h for h in required_headers.keys() if h in response.headers],
                    "missing_headers": missing_headers
                },
                error=f"Missing security headers: {', '.join(missing_headers)}" if missing_headers else None
            ))
...
async def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Validate privacy guarantees"
    )
    parser.add_argument(
        "--api-key",
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Maximum retries for failed tests"
    )
