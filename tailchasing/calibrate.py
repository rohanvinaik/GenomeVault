...
class CalibrationTool:
    ...
    def fit_parameters(
        ...
        if not historical_thrash_events:
            raise ValueError("Need at least one historical thrash event for calibration")
        ...
        optimal_alpha, best_alpha_score = self._grid_search_alpha(X, y, alpha_range, alpha_steps)

        self.logger.info(f"Optimal alpha: {optimal_alpha:.4f} (score: {best_alpha_score:.4f})")
        ...
        self.logger.info(f"Calibration complete. Validation score: {validation_score:.4f}")
    ...
    def _calculate_token_distance(self, event: ThrashEvent, codebase: CodebaseMetrics) -> float:
    ...
    def _calculate_ast_distance(self, event: ThrashEvent, codebase: CodebaseMetrics) -> float:
    ...
    def _calculate_module_distance(self, event: ThrashEvent, codebase: CodebaseMetrics) -> float:
    ...
    def _calculate_git_distance(self, event: ThrashEvent, codebase: CodebaseMetrics) -> float:
    ...
    def _grid_search_alpha(
        ...
        for alpha in alphas:
            try:
                predictions = self._predict_thrash_probability(X, alpha, default_weights)
...
def create_sample_codebase_metrics(file_paths: List[str]) -> CodebaseMetrics:
    ...
    for file_path in file_paths:
        ...
        deps = np.random.choice(file_paths, size=min(dep_count, len(file_paths) - 1), replace=False)
