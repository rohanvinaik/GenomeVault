...
class PolymerConfig:
    ...
    @classmethod
    def load_or_default(cls, config_path: Optional[Union[str, Path]] = None) -> "PolymerConfig":
        """
...
class ConfigManager:
    ...
    def save_config(self, config: PolymerConfig, path: Optional[Union[str, Path]] = None) -> None:
        """Save configuration to file."""
        save_path = (
            Path(path) if path else (self.config_path or PolymerConfig.get_default_config_path())
        )
    ...
    def validate_config(self, config: PolymerConfig) -> List[str]:
        ...
        if not (
            config.weights.tok <= config.weights.ast <= config.weights.mod <= config.weights.git
        ):
            issues.append(
...
def save_config(config: PolymerConfig, config_path: Optional[Union[str, Path]] = None) -> None:
