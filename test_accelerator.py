...
from genomevault.accelerator import Accelerator, get_accelerator
...
class TestAccelerator(unittest.TestCase):
    ...
    def test_consistency_across_implementations(self):
        ...
        # Test all operations
        tests = [
            ('hypervector_similarity', 
             lambda a: a.hypervector_similarity(vec1, vec2)),
            ('hamming_distance',
             lambda a: a.hamming_distance(binary1, binary2)),
            ('pir_xor_mask',
             lambda a: a.pir_xor_mask(binary1, binary2)),
            ('encode_variant',
             lambda a: a.encode_variant(1, 12345, "A", "G", 1000)),
        ]
