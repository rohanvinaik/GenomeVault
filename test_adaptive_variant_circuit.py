...
def benchmark_circuit(circuit, name: str, test_sizes: list):
    ...
    for size in test_sizes:
        ...
        print(f"  Size {size:4d}: {avg_time:6.3f}ms ± {std_time:4.3f}ms (found: {witness.get('found', False)})")
...
def main():
    ...
    print(f"Original threshold: 50 variants")
    ...
    for size in [1, 5, 10, 25, 50, 75, 100]:
        ...
        if adaptive_time:
            if small_time < float('inf') and large_time < float('inf'):
                ...
                overhead = (adaptive_time - optimal_time) / optimal_time * 100 if optimal_time > 0 else 0
                print(f"{size:4d} | {small_time:6.3f}ms | {large_time:6.3f}ms | {adaptive_time:7.3f}ms | {best:5s} (+{overhead:.1f}%)")
