...
def test_hdc_integration():
    ...
    config_no_dp = HypervectorConfig(
        dimension=1000,
...
def test_federated_integration():
    ...
    dp_fed = DifferentiallyPrivateFederated(
        num_clients=10,
        privacy_level=PrivacyLevel.COMMON
    )
...
def test_pir_integration():
    ...
    # Initialize DP-PIR
    dp_pir = DifferentiallyPrivatePIR(
        database_size=1000,
        privacy_level=PrivacyLevel.COMMON
    )
    ...
    for query_type in query_types:
        ...
        # Add DP noise
        noisy_response = dp_pir.add_noise_to_response(
            original_response.copy(),
            query_type
        )
        ...
        print(f"  Original: {original_response[0] if len(original_response) == 1 else f'vector[{len(original_response)}]'}")
        print(f"  Noisy:    {noisy_response[0] if len(noisy_response) == 1 else f'vector[{len(noisy_response)}]'}")
