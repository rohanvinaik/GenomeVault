...
from genomevault.zk_proofs.circuits.optimized.diabetes_risk_alert import OptimizedDiabetesRiskCircuit
from genomevault.zk_proofs.circuits.adaptive_variant import AdaptiveVariantPresenceCircuit as AdaptiveVariantCircuit
from genomevault.zk_proofs.witness_cache import WitnessCache, get_witness_cache
...
from genomevault.zk_proofs.dashboard import PerformanceDashboard
...
class PipelineProfiler:
    ...
    def print_summary(self):
        ...
        for name, stats in sorted_results:
            ...
            print(f"{name:34s} | {stats['time_ms']:9.2f} | {stats['memory_delta_mb']:+6.1f} | {status} {pct:5.1f}%")
...
def test_3_adaptive_circuit_selection():
    ...
    large_variants = [
        {'chr': f'chr{i%22+1}', 'pos': i*1000, 'alt': 'G'}
        for i in range(100)
    ]
...
def test_6_memory_pool():
    ...
    witness = mem_prover.generate_witness_with_pool(
        'variant_presence',
        {'variant_hash': 'test'},
        {'variant_data': {'chr': '1', 'pos': 12345}}
    )
    
    if hasattr(witness, 'metadata') and witness.metadata.get('memory_pool_used'):
        print(f"  ✅ Memory pool used for witness generation")
...
def test_7_gpu_acceleration():
    ...
    if gpu_prover:
        ...
        print(f"  Optimized settings: batch_size={settings['batch_size']}, "
...
def identify_optimization_opportunities(profiler: PipelineProfiler):
    ...
    for name, stats in profiler.results.items():
        ...
        if stats['memory_delta_mb'] > 100:
            opportunities.append(f"Optimize memory usage in {name} (using {stats['memory_delta_mb']:.1f}MB)")
