...
from genomevault.observability.metrics.prometheus import get_metrics_collector, get_prometheus_metrics
from genomevault.observability.logging.structured import get_structured_logger, set_request_context, generate_request_id
...
async def test_opentelemetry_tracing():
    ...
    with tracing_manager.trace_operation(
        "test_operation",
        attributes={"test.attribute": "test_value", "test.number": 42}
    ) as span:
        if span:
...
def test_integration():
    ...
    logger.info("Completed integrated genomic operation",
