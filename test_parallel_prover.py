...
def main():
    ...
    for workers in worker_counts:
        ...
        speedup = thread_results['throughput'] / seq_results['throughput'] if seq_results['throughput'] > 0 else 0
        ...
        print(f"{workers} threads: {thread_results['throughput']:.1f} proofs/sec (speedup: {speedup:.1f}x)")
    ...
    for r in results_table:
        print(f"{r['workers']:7d} | {r['type']:7s} | {r['time']:8.2f} | {r['throughput']:10.1f} | {r['speedup']:7.1f}x | {r['queue_time']:9.2f}")
