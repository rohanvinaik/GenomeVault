...
from genomevault.api.middleware.input_sanitization import (
    InputSanitizer, DataType, GenomicInputSanitizer
)
from genomevault.api.middleware.audit_logging import AuditLogger, AuditEventType, AuditSeverity
from genomevault.api.middleware.cors_security import (
    OriginValidator, CORSSecurityLevel, validate_origin_security
)
from genomevault.api.middleware.security_headers import (
    SecurityHeadersConfig, SecurityProfile, validate_security_headers
)
...
def test_rate_limiting():
    ...
    assert clinical_config.requests_per_minute < public_config.requests_per_minute, \
...
def test_security_headers():
    ...
    assert len(issues) <= 1, f"Valid headers should have minimal issues: {issues}"  # May have HSTS missing
...
def test_integration():
    ...
    audit_authentication(
        ...
        details={'key_type': key_info.key_type.value}
    )
    
    # Test failed authentication audit
    audit_authentication(
        success=False,
        actor_id="invalid_key_123",
        details={'failure_reason': 'invalid_key'}
    )
