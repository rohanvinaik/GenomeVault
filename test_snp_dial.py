...
def demonstrate_snp_dial():
    ...
    for granularity, description in panel_settings:
        ...
        encoder = GenomicEncoder(
            dimension=100000, enable_snp_mode=enable_snp, panel_granularity=granularity
        )
    ...
    for granularity in ["common", "clinical"]:
        if granularity in results:
            similarity = torch.cosine_similarity(
                base_encoding, results[granularity]["encoded_genome"], dim=0
            ).item()
    ...
    encoder = GenomicEncoder(
        dimension=100000, enable_snp_mode=True, panel_granularity=PanelGranularity.CLINICAL
    )
...
"""
    ...
    try:
        ...
        encoder = GenomicEncoder(
            dimension=100000, enable_snp_mode=True, panel_granularity=PanelGranularity.OFF
        )
...
def benchmark_panel_scaling():
    ...
    for dim, sparsity, desc in configs:
        encoder = GenomicEncoder(
            dimension=dim, enable_snp_mode=True, panel_granularity=PanelGranularity.CLINICAL
        )
...
def demonstrate_proof_aggregation():
    ...
    for scenario in scenarios:
        print(
            f"{scenario['name']:<30} {scenario['panel']:<10} "
