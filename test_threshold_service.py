...
def test_threshold_signing(service: ThresholdCryptoService, participants: list):
    ...
    signature = service.create_threshold_signature(
        message=message1, initiator=participants[0], participants=signers
    )
    ...
    signature2 = service.create_threshold_signature(
        message=message1, initiator=participants[0], participants=insufficient_signers
    )
    ...
    for i in range(15):  # Try more than rate limit
        try:
            session_id = service.signing.initiate_signing(
                message=f"Test message {i}".encode(), participant_id=participants[0]
            )
...
def test_threshold_encryption(service: ThresholdCryptoService, participants: list):
    ...
    genomic_data = json.dumps(
        {"patient_id": "P12345", "variants": ["BRCA1", "BRCA2", "TP53"], "risk_score": 0.87}
    ).encode()
    ...
    ciphertext, encrypted_shares = service.encrypt_for_threshold(
        data=genomic_data, participants=participants[: service.config.total_shares]
    )
...
def test_recovery_mechanism(service: ThresholdCryptoService):
    ...
    recovered_secret = service.execute_recovery(
        recovery_id="emergency_001", shares=collected_shares, commitments=commitments
    )
    ...
    recovered2 = service.execute_recovery(
        recovery_id="emergency_002", shares=same_region_shares, commitments=commitments
    )
...
def test_session_management(service: ThresholdCryptoService):
    ...
    for i in range(5):
        test_service.register_participant(f"test_{i}", hashlib.sha256(f"key_{i}".encode()).digest())

    # Initiate signing session
    session_id = test_service.signing.initiate_signing(b"Test message", "test_0")
    ...
    try:
        test_service.signing.submit_signature_share(session_id, "test_1", b"late_signature")
