...
from genomevault.compression.tiered_compression import (
    CompressionTier,
    TieredCompressor,
    VariantPriority,
    CompressionMetrics
...
def test_clinical_tier():
    ...
    # Verify size target
    assert metrics.compressed_size <= CompressionTier.CLINICAL.target_bytes * 1.1, \
        f"CLINICAL compression exceeds target: {metrics.compressed_size} > {CompressionTier.CLINICAL.target_bytes}"
    
    # Verify quality metrics
    assert metrics.information_retention >= 0.7, \
        f"Information retention too low: {metrics.information_retention}"
    
    assert metrics.clinical_coverage >= 0.9, \
        f"Clinical coverage too low: {metrics.clinical_coverage}"
...
def test_full_hdc_tier():
    ...
    assert 100*1024 <= metrics.compressed_size <= 200*1024, \
        f"FULL_HDC compression outside target range: {metrics.compressed_size}"
    
    # Verify quality metrics
    assert metrics.information_retention >= 0.85, \
        f"Information retention too low: {metrics.information_retention}"
    
    assert metrics.clinical_coverage >= 0.75, \
        f"Clinical coverage too low: {metrics.clinical_coverage}"
...
def test_variant_selection():
    ...
    for tier in CompressionTier:
        ...
        if tier == CompressionTier.MINI:
            ...
            assert common_count > 1000, f"MINI should prioritize common variants"
            
        elif tier == CompressionTier.CLINICAL:
            assert len(variants) <= 120000, f"CLINICAL should select ≤120k variants"
            assert acmg_count > 0 or pharmgkb_count > 0, "CLINICAL should include ACMG/PharmGKB"
            
        else:  # FULL_HDC
            assert len(variants) == 10000, f"FULL_HDC should select 10000 variants"
...
def test_multi_modal_storage():
    ...
    assert total_storage == expected_total, \
        f"Storage calculation mismatch: {total_storage} != {expected_total}"
    
    # Verify reasonable storage for mobile device
    assert total_storage < 1024 * 1024, \
        f"Total storage exceeds 1MB: {total_storage}"
...
def test_compression_quality():
    ...
    for tier in CompressionTier:
        ...
        assert metrics.reconstruction_accuracy >= 0.7, \
