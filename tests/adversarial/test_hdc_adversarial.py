...
class TestPrivacyAttacks:
    ...
    def test_membership_inference_attack(self):
        ...
        for train_hv in train_hvs:
            sims = [encoder.similarity(train_hv, other) for other in train_hvs if other is not train_hv]
        ...
        assert abs(train_mean - test_mean) / train_mean < 0.1, \
    ...
    def test_model_inversion_attack(self):
        ...
        assert abs(recovery_correlation) < 0.3, \
...
class TestSystematicVulnerabilities:
    ...
    def test_timing_side_channel(self):
        ...
        timing_variance = np.var(timings) / np.mean(timings)**2  # Coefficient of variation squared
...
class TestCryptographicProperties:
    ...
    def test_collision_resistance_adversarial(self):
        ...
        for i in range(num_attempts):
            ...
            # Check for collision
            assert hv_bytes not in encoded_set, \
                f"Collision found at attempt {i}"
