...
class TestTimingAttacks:
    ...
    async def test_constant_response_time(self):
        ...
        servers = [
            PIRServer(f"server_{i}", f"http://localhost:900{i}", 
                     "region", False, 0.95, 50)
            for i in range(3)
        ]
...
class TestCollusion:
    ...
    def test_collusion_detection_simulation(self):
        ...
        colluding_sum = np.bitwise_xor(
            server_queries["server_1"],
            server_queries["server_2"]
        )
...
class TestReplayAttacks:
    ...
    async def test_nonce_replay_detection(self):
        ...
        server = Mock()
        server.process_query = asyncio.coroutine(lambda x: {
            "response": [0] * 1024,
        ...
        request1 = Mock()
        request1.json = asyncio.coroutine(lambda: {
            "query_id": "test-123",
            "query_vector": [0] * 1000,
            "vector_size": 1000,
            "timestamp": time.time(),
        ...
        # Replay same nonce
        request2 = Mock()
        request2.json = asyncio.coroutine(lambda: {
