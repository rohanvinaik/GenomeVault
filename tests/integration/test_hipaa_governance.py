...
from genomevault.blockchain.governance import CommitteeType
from genomevault.blockchain.governance import GovernanceSystem
from genomevault.blockchain.governance import ProposalStatus
from genomevault.blockchain.governance import ProposalType
from genomevault.blockchain.hipaa import CMSNPIRegistry
from genomevault.blockchain.hipaa import HIPAACredentials
from genomevault.blockchain.hipaa import HIPAAVerifier
from genomevault.blockchain.hipaa.integration import HIPAAGovernanceIntegration
from genomevault.blockchain.hipaa.integration import HIPAANodeIntegration
...
class TestHIPAANodeIntegration:
    ...
    async def test_provider_node_registration(self):
        ...
        async with CMSNPIRegistry() as registry:
            ...
            node_config = {"node_class": NodeType.FULL, "location": "US-East"}
        ...
        async with CMSNPIRegistry() as registry:
            ...
            for npi, node_type, expected_power in providers:
                ...
                node = await integration.register_provider_node(
                    credentials, {"node_class": node_type}
                )
    ...
    async def test_provider_node_revocation(self):
        ...
        async with CMSNPIRegistry() as registry:
            ...
            node = await integration.register_provider_node(
                credentials, {"node_class": NodeType.FULL}
            )
        ...
        async with CMSNPIRegistry() as registry:
            ...
            for npi in npis:
                ...
                await integration.register_provider_node(
                    credentials, {"node_class": NodeType.LIGHT}
                )
...
class TestHIPAAGovernance:
    ...
    async def test_hipaa_enhanced_voting(self):
        ...
        async with CMSNPIRegistry() as registry:
            ...
            base_weight = governance.voting_mechanism.calculate_vote_weight(
                node.voting_power, "yes"
            )
    ...
    async def test_complete_provider_journey(self):
        ...
        async with CMSNPIRegistry() as registry:
            ...
            provider_vote = governance.vote(
                proposal_id=proposal.proposal_id, voter=node.node_id, choice="yes"
            )
