...
def feature_arrays(draw, min_size=10, max_size=10000):
    ...
    return draw(arrays(dtype=np.float32, shape=(size,), 
...
def binding_compatible_vectors(draw, dimension):
    ...
    for _ in range(num_vectors):
        vec = draw(arrays(dtype=np.float32, shape=(dimension,),
...
class TestHDCProperties:
    ...
    def test_similarity_preservation_property(self, dimension, features1, features2):
        ...
        orig_sim = np.corrcoef(features1[:min(len(features1), len(features2))], 
                               features2[:min(len(features1), len(features2))])[0, 1]
...
class TestBindingProperties:
    ...
    @given(
        ...
        vectors=st.integers(min_value=2, max_value=10).flatmap(
            lambda n: st.lists(
                arrays(dtype=np.float32, shape=(10000,), 
                      elements=st.floats(-10, 10, allow_nan=False, allow_infinity=False)),
    ...
    @given(
        dimension=st.integers(min_value=1000, max_value=20000)
    )
    @settings(max_examples=30)
    ...
    @given(
        dimension=st.integers(min_value=1000, max_value=5000)
    )
    @settings(max_examples=20)
...
class TestCompressionProperties:
    ...
    @given(
        features=feature_arrays(min_size=100, max_size=5000)
    )
    @settings(max_examples=20)
...
class TestPerformanceProperties:
    ...
    @given(
        dimension=st.sampled_from([1000, 5000, 10000])
    )
