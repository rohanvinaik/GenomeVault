...
class TestCompressionTiers:
    ...
    def sample_snp_data(self):
        ...
        mini_snps = {
            f"rs{i}": np.random.choice(['AA', 'AT', 'TT'], p=[0.25, 0.5, 0.25])
            for i in range(5000)
        }
        
        # Clinical tier: ACMG + PharmGKB variants (~120k)
        clinical_snps = {
            f"rs{i}": np.random.choice(['AA', 'AT', 'TT', 'GG', 'GC', 'CC'], 
    ...
    def test_combined_storage_calculation(self, sample_snp_data, sample_hypervector):
        ...
        tiered.add_modality('pharmacogenomics', CompressionTier.CLINICAL, 
                           sample_snp_data['clinical'])
        
        total_size = tiered.get_total_size()
        expected = 25 + 300  # KB
        
        assert abs(total_size - expected) < expected * 0.1, \
            f"Combined size {total_size}KB differs from expected {expected}KB"
    
    @pytest.mark.parametrize("tier,expected_features,expected_size", [
    ...
    def test_tier_specifications(self, tier, expected_features, expected_size):
        ...
        compressor = SNPCompressor(tier=tier) if tier != CompressionTier.FULL_HDC \
                     else HDCCompressor()
        
        # Verify tier metadata
        assert compressor.get_feature_count() == expected_features or \
               str(expected_features) in str(compressor.get_feature_description())
