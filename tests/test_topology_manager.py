...
class TestTopologyManager:
    ...
    def sample_servers(self) -> List[Server]:
        ...
        servers = [
            # Light Nodes
            Server("ln1", ServerType.LIGHT_NODE, "http://ln1.example.com", 
                  latency_ms=50.0, reliability=0.99, capacity=100),
            Server("ln2", ServerType.LIGHT_NODE, "http://ln2.example.com",
                  latency_ms=60.0, reliability=0.98, capacity=100),
            Server("ln3", ServerType.LIGHT_NODE, "http://ln3.example.com",
                  latency_ms=70.0, reliability=0.97, capacity=100),
            
            # Trusted Signatories
            Server("ts1", ServerType.TRUSTED_SIGNATORY, "http://ts1.example.com",
                  latency_ms=40.0, reliability=0.995, capacity=50),
            Server("ts2", ServerType.TRUSTED_SIGNATORY, "http://ts2.example.com",
                  latency_ms=45.0, reliability=0.995, capacity=50),
            Server("ts3", ServerType.TRUSTED_SIGNATORY, "http://ts3.example.com",
                  latency_ms=55.0, reliability=0.99, capacity=50),
            
            # Full Nodes
            Server("fn1", ServerType.FULL_NODE, "http://fn1.example.com",
                  latency_ms=80.0, reliability=0.98, capacity=200),
        ]
        return servers
    ...
    def test_server_selection(self, topology_manager, sample_servers):
        ...
        trusted_count = sum(1 for s in selected 
        ...
        trusted_servers = [s for s in selected 
                          if s.type == ServerType.TRUSTED_SIGNATORY]
    ...
    def test_failure_probability(self, topology_manager, sample_servers):
        ...
        high_reliability_servers = [
            Server(f"s{i}", ServerType.LIGHT_NODE, f"http://s{i}.com",
                  latency_ms=50, reliability=0.999, capacity=100)
            for i in range(3)
        ]
    ...
    def test_exploration_vs_exploitation(self, adaptive_manager, sample_servers):
        ...
        for i in range(101):
            ...
            adaptive_manager.record_performance(
                adaptive_manager.current_preset,
                250.0 + (i % 50),
                True
            )
