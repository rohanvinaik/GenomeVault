...
from genomevault.pir.server.enhanced_pir_server import (
    EnhancedPIRServer, OptimizedPIRDatabase, ShardMetadata,
    GenomicRegion, TrustedSignatoryServer
)
...
class TestEnhancedPIRServer:
    ...
    async def test_process_query(self, server):
        ...
        query = {
            ...
            'query_vectors': [
                np.zeros(1000, dtype=np.uint8)  # All zeros for simplicity
            ],
            'query_type': 'genomic',
            'parameters': {
                'regions': [
...
class TestPIRIntegration:
    ...
    async def test_multi_server_query(self, tmp_path):
        ...
        query_vectors = [
            np.random.binomial(1, 0.001, 100).astype(np.uint8)
            for _ in range(3)
        ]
