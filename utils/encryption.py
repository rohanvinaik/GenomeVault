...
class AESGCMCipher:
    ...
    @classmethod
    def encrypt(cls, plaintext: bytes, key: bytes, 
    ...
    @classmethod
    def decrypt(cls, ciphertext: bytes, key: bytes, nonce: bytes, tag: bytes,
...
class ThresholdCrypto:
    ...
    @classmethod
    def split_secret(cls, secret: bytes, threshold: int, 
                    total_shares: int) -> List[ThresholdShare]:
        """
    ...
    def reconstruct_secret(cls, shares: List[ThresholdShare]) -> bytes:
        ...
        for share in shares[:threshold]:
            ...
            if not constant_time.bytes_eq(
                ...
                raise ValueError(f"Invalid checksum for share {share.share_id}")
...
class KeyDerivation:
    ...
    @classmethod
    def derive_key_hkdf(cls, input_key: bytes, info: bytes, 
...
class HomomorphicHelper:
    ...
    def prepare_for_homomorphic(cls, value: int, bit_length: int = 64) -> bytes:
        ...
        if value >= 2**bit_length:
            raise ValueError(f"Value too large for {bit_length} bits")
