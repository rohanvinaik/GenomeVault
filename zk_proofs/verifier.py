...
class Verifier:
    ...
    def verify_proof(self, proof: Proof) -> VerificationResult:
        ...
        try:
            ...
            logger.info(f"Proof verification completed: {is_valid}", 
    ...
    def _verify_variant_proof(self, proof: Proof) -> bool:
        ...
        try:
            ...
            proof_data = json.loads(proof.proof_data.decode()) if isinstance(proof.proof_data, bytes) else proof.proof_data
    ...
    def _verify_prs_proof(self, proof: Proof) -> bool:
        ...
        try:
            ...
            if not isinstance(score_range, dict) or 'min' not in score_range or 'max' not in score_range:
                return False
    ...
    def batch_verify(self, proofs: List[Proof]) -> List[VerificationResult]:
        ...
        logger.info(f"Batch verification: {valid_count}/{len(proofs)} valid", 
                   extra={'privacy_safe': True})
        
        return results
    
    def verify_recursive_proof(self, recursive_proof: Proof, 
                             original_proofs: List[Proof]) -> VerificationResult:
        """
